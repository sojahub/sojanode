...
import (
	...
	"github.com/Sojahub/sojanode/app/ante"
...
func TestAdjustGasPriceDecorator_AnteHandle(t *testing.T) {
	...
	addrs := sojaapp.AddTestAddrs(app, ctx, 6, initTokens)
	...
	otherMsg := banktypes.NewMsgSend(addrs[0], addrs[1], sdk.NewCoins(sdk.NewCoin("fury", sdk.NewIntFromUint64(100))))
...
func TestAdjustGasPriceDecorator_AnteHandle_MinFee(t *testing.T) {
	...
	addrs := sojaapp.AddTestAddrs(app, ctx, 6, initTokens)
	decorator := ante.NewAdjustGasPriceDecorator(app.AdminKeeper)
	highFee := sdk.NewCoins(sdk.NewCoin("fury", sdk.NewInt(100000000000000000))) // 0.1
	lowFee := sdk.NewCoins(sdk.NewCoin("fury", sdk.NewInt(10000000000000000)))   // 0.01

	sendMsg := banktypes.NewMsgSend(addrs[0], addrs[1], sdk.NewCoins(sdk.NewCoin("fury", sdk.NewIntFromUint64(100))))
