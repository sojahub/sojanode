...
import (
	...
	"github.com/Sojahub/sojanode/tools/sojagen/common"
	"github.com/Sojahub/sojanode/tools/sojagen/genesis"
	"github.com/Sojahub/sojanode/tools/sojagen/network/types"
...
func Reset(chainID, networkDir string) error {
	...
	for _, node := range nodes {
		nodeDir := fmt.Sprintf("%s/validators/%s/%s/.sojanoded", networkDir, chainID, node.Moniker)
...
func (n *Network) generateKey(validator *Validator) error {
	output, err := n.CLI.AddKey(validator.Moniker, validator.Mnemonic, validator.Password, fmt.Sprintf("%s/%s", validator.HomeDir, ".sojanoded"))
	if err != nil {
		return err
	}
...
func (n *Network) addValidatorKeyToSeed(validator, seedValidator *Validator) error {
	_, err := n.CLI.AddKey(validator.Moniker, validator.Mnemonic, seedValidator.Password, fmt.Sprintf("%s/%s", seedValidator.HomeDir, ".sojanoded"))
	if err != nil {
		return err
	}
