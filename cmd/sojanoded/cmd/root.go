...
func createSimappAndExport(logger log.Logger, db dbm.DB, traceStore io.Writer, height int64, forZeroHeight bool,
	...
	var sojaApp *app.SojahubApp
	if height != -1 {
		sojaApp = app.NewSojaApp(logger, db, traceStore, false, map[int64]bool{}, "", cast.ToUint(appOpts.Get(server.FlagInvCheckPeriod)), encCfg, appOpts)
		if err := sojaApp.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		sojaApp = app.NewSojaApp(logger, db, traceStore, true, map[int64]bool{}, "", cast.ToUint(appOpts.Get(server.FlagInvCheckPeriod)), encCfg, appOpts)
	}
	return sojaApp.ExportAppStateAndValidators(forZeroHeight, jailAllowedAddrs)
}
