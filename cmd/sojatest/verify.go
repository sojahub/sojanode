...
func VerifyAdd(clientCtx client.Context, from string, height uint64, nativeAmount, externalAmount sdk.Uint, externalAsset string) error {
	...
	furyBefore, err := bankQueryClient.Balance(context.Background(), &banktypes.QueryBalanceRequest{
		Address: from,
		Denom:   "fury",
	})
	if err != nil {
	...
	furyAfter, err := bankQueryClient.Balance(context.Background(), &banktypes.QueryBalanceRequest{
		Address: from,
		Denom:   "fury",
	})
	if err != nil {
	...
	nativeDiff := furyAfter.Balance.Amount.Sub(furyBefore.Balance.Amount)
...
func VerifyClose(clientCtx client.Context, from string, height int64, id uint64) error {
	...
	if types.StringCompare(mtpResponse.Mtp.CollateralAsset, "fury") {
	...
	if types.StringCompare(mtpResponse.Mtp.CustodyAsset, "fury") {
	...
	X, Y, toFury, _ := pool.ExtractValues(clptypes.Asset{Symbol: mtpResponse.Mtp.CollateralAsset})
	X, Y = pool.ExtractDebt(X, Y, toFury)
	repayAmount, _ := clpkeeper.CalcSwapResult(toFury, X, mtpResponse.Mtp.CustodyAmount, Y, pmtpParams.PmtpRateParams.PmtpCurrentRunningRate, swapFeeParams.DefaultSwapFeeRate)
