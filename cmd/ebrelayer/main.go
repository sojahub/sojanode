...
import (
	...
	ebrelayertypes "github.com/Sojahub/sojanode/cmd/ebrelayer/types"
	flag "github.com/spf13/pflag"

	sojaapp "github.com/Sojahub/sojanode/app"
	"github.com/Sojahub/sojanode/cmd/ebrelayer/contract"
	"github.com/Sojahub/sojanode/cmd/ebrelayer/relayer"
...
func buildRootCmd() *cobra.Command {
	// see cmd/sojanoded/cmd/root.go:37 ; we need to do the
	// same thing in ebrelayer
	encodingConfig := sojaapp.MakeTestEncodingConfig()
	initClientCtx := client.Context{}.
		...
		WithHomeDir(sojaapp.DefaultNodeHome)
	...
	rootCmd.PersistentFlags().String(
		...
		"",
		"Path to a json file containing an array of sojahub denom => Ethereum symbol pairs",
	)
	rootCmd.PersistentFlags().String(
...
func replayEthereumCmd() *cobra.Command {
	...
	replayEthereumCmd := &cobra.Command{
		Use:     "replayEthereum [tendermintNode] [web3Provider] [bridgeRegistryContractAddress] [validatorMoniker] [validatorMnemonic] [fromBlock] [toBlock] [sojaFromBlock] [sojaEndBlock]",
...
func main() {
	if err := svrcmd.Execute(buildRootCmd(), sojaapp.DefaultNodeHome); err != nil {
		switch e := err.(type) {
