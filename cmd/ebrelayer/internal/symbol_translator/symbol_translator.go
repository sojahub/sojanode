...
import (
	"encoding/json"
	bimap2 "github.com/Sojahub/sojanode/cmd/ebrelayer/internal/bimap_with_default"
	"github.com/vishalkuo/bimap"
	"io/ioutil"
)

// SymbolTranslator translates between Sojahub denoms and Ethereum symbols
type SymbolTranslator struct {
	symbolTable *bimap.BiMap
}

func (s *SymbolTranslator) SojahubToEthereum(denom string) string {
	return bimap2.GetWithDefault(s.symbolTable, denom, denom).(string)
}

func (s *SymbolTranslator) EthereumToSojahub(symbol string) string {
