...
import (
	...
	"github.com/Sojahub/sojanode/x/oracle/types"
)
...
func (k Keeper) EnsureAddressIsInWhitelist(ctx sdk.Context, validatorAddress string) error {
	...
	k.Logger(ctx).Error(
		"sojanode oracle keeper ProcessClaim validator not in whitelist",
...
func (k Keeper) ProcessClaim(ctx sdk.Context, claim types.Claim) (types.Status, error) {
	...
	if !activeValidator {
		logger.Error("sojanode oracle keeper ProcessClaim validator not active.")
		return types.Status{}, types.ErrInvalidValidator
	}
	if claim.Id == "" {
		logger.Error("sojanode oracle keeper ProcessClaim wrong claim id.", "claimID", claim.Id)
		return types.Status{}, types.ErrInvalidIdentifier
	}
	if claim.Content == "" {
		logger.Error("sojanode oracle keeper ProcessClaim claim content is empty.")
