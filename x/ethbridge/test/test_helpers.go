...
import (
	...
	"github.com/Sojahub/sojanode/x/ethbridge/keeper"
...
func CreateTestApp(isCheckTx bool) (*app.SojahubApp, sdk.Context) {
	sojahubApp := app.Setup(isCheckTx)
	ctx := sojahubApp.BaseApp.NewContext(isCheckTx, tmproto.Header{})
	sojahubApp.AccountKeeper.SetParams(ctx, authtypes.DefaultParams())
	initTokens := sdk.TokensFromConsensusPower(1000, sdk.DefaultPowerReduction)
	_ = app.AddTestAddrs(sojahubApp, ctx, 6, initTokens)
	return sojahubApp, ctx
}
...
func CreateSimulatorApp(isCheckTx bool) (sdk.Context, *app.SojahubApp) {
	sojahubApp := app.Setup(isCheckTx)
	ctx := sojahubApp.BaseApp.NewContext(isCheckTx, tmproto.Header{})
	sojahubApp.TokenRegistryKeeper.SetRegistry(ctx, tokenregistryTypes.Registry{
	...
	return ctx, sojahubApp
}

func CreateTestAppEthBridge(isCheckTx bool) (sdk.Context, keeper.Keeper) {
	sojaapp, ctx := CreateTestApp(isCheckTx)
	return ctx, sojaapp.EthbridgeKeeper
}
