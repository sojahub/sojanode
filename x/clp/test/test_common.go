...
func CreateTestAppClp(isCheckTx bool) (sdk.Context, *sojaapp.SojahubApp) {
	return CreateTestAppClpWithBlacklist(isCheckTx, []sdk.AccAddress{})
}

func CreateTestAppClpWithBlacklist(isCheckTx bool, blacklist []sdk.AccAddress) (sdk.Context, *sojaapp.SojahubApp) {
	sojaapp.SetConfig(false)
	app := sojaapp.SetupWithBlacklist(isCheckTx, blacklist)
	ctx := app.BaseApp.NewContext(isCheckTx, tmproto.Header{})
	...
	liquidityProtectionParam.MaxFuryLiquidityThreshold = sdk.ZeroUint()
...
func CreateTestAppClpFromGenesis(isCheckTx bool, genesisTransformer func(*sojaapp.SojahubApp, sojaapp.GenesisState) sojaapp.GenesisState) (sdk.Context, *sojaapp.SojahubApp) {
	sojaapp.SetConfig(false)
...
func GenerateRandomLP(numberOfLp int) []types.LiquidityProvider {
	...
	for i := 0; i < numberOfLp; i++ {
		externalToken := tokens[rand.Intn(len(tokens))]
		asset := types.NewAsset(TrimFirstRune(externalToken))
		lpAddess, err := sdk.AccAddressFromBech32("did:fury:s1azpar20ck9lpys89r8x7zc8yu0qzgvtpgpurs6")
		if err != nil {
			panic(err)
		}
...
func GeneratePoolsFromFile(app *sojaapp.SojahubApp, keeper clpkeeper.Keeper, ctx sdk.Context) []*types.Pool {
	...
	for _, pool := range poolList.Pools {
		...
		err = app.BankKeeper.MintCoins(ctx, types.ModuleName, sdk.NewCoins(
			sdk.NewCoin("fury", sdk.NewIntFromBigInt(pool.NativeAssetBalance.BigInt())),
