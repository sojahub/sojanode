...
func GetCmdUpdateLiquidityProtectionParams() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msg := types.MsgUpdateLiquidityProtectionParams{
				Signer:                          signer.String(),
				MaxFuryLiquidityThreshold:      sdk.NewUintFromString(viper.GetString(FlagMaxFuryLiquidityThreshold)),
				MaxFuryLiquidityThresholdAsset: viper.GetString(FlagMaxFuryLiquidityThresholdAsset),
	...
	cmd.Flags().AddFlagSet(FsMaxFuryLiquidityThreshold)
	cmd.Flags().AddFlagSet(FsMaxFuryLiquidityThresholdAsset)
	cmd.Flags().AddFlagSet(FsPmtpPeriodEpochLength)
	cmd.Flags().AddFlagSet(FsLiquidityThresholdIsActive)
	if err := cmd.MarkFlagRequired(FlagPmtpPeriodEpochLength); err != nil {
		log.Println("MarkFlagRequired  failed: ", err.Error())
	}
	if err := cmd.MarkFlagRequired(FlagMaxFuryLiquidityThreshold); err != nil {
		log.Println("MarkFlagRequired  failed: ", err.Error())
	}
	if err := cmd.MarkFlagRequired(FlagMaxFuryLiquidityThresholdAsset); err != nil {
		log.Println("MarkFlagRequired  failed: ", err.Error())
	}
...
func GetCmdModifyLiquidityProtectionRates() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short: "Modify liquidity protection current fury liquidity threshold",
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
			if err != nil {
				return err
			...
			msg := types.MsgModifyLiquidityProtectionRates{
				Signer:                         signer.String(),
				CurrentFuryLiquidityThreshold: sdk.NewUintFromString(viper.GetString(FlagCurrentFuryLiquidityThreshold)),
			}
			if err := msg.ValidateBasic(); err != nil {
				return err
			}
	...
	cmd.Flags().AddFlagSet(FsCurrentFuryLiquidityThreshold)
	if err := cmd.MarkFlagRequired(FlagCurrentFuryLiquidityThreshold); err != nil {
		log.Println("MarkFlagRequired  failed: ", err.Error())
	}
	flags.AddTxFlagsToCmd(cmd)
	return cmd
