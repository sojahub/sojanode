...
func TestCreatePool(t *testing.T) {
	...
	err := sojaapp.AddCoinsToAccount(clptypes.ModuleName, app.BankKeeper, ctx, signer, sdk.NewCoins(externalCoin, nativeCoin))
	...
	err = sojaapp.AddCoinsToAccount(clptypes.ModuleName, app.BankKeeper, ctx, signer, sdk.Coins{newAssetCoin}.Sort())
	require.NoError(t, err)
	// Create Pool
...
func TestAddLiquidity(t *testing.T) {
	...
	err = sojaapp.AddCoinsToAccount(clptypes.ModuleName, app.BankKeeper, ctx, signer2, sdk.NewCoins(externalCoin, nativeCoin))
	require.NoError(t, err)
...
func TestAddLiquidity_LargeValue(t *testing.T) {
	...
	handler := clp.NewHandler(clpKeeper)
	//Parameters for add liquidity
	poolBalanceFury := sdk.NewUintFromString("162057826929020210025062784")
	...
	addLiquidityAmountFury := sdk.NewUintFromString("1000000000000000000000")
	...
	nativeCoin := sdk.NewCoin(clptypes.NativeSymbol, sdk.Int(poolBalanceFury).Add(sdk.Int(addLiquidityAmountFury)))
	err := sojaapp.AddCoinsToAccount(clptypes.ModuleName, app.BankKeeper, ctx, signer, sdk.NewCoins(externalCoin, nativeCoin))
	require.NoError(t, err)
	msgCreatePool := clptypes.NewMsgCreatePool(signer, asset, poolBalanceFury, poolBalanceCacoin)
	res, err := handler(ctx, &msgCreatePool)
	require.NoError(t, err)
	require.NotNil(t, res)
	msg := clptypes.NewMsgAddLiquidity(signer, asset, addLiquidityAmountFury, addLiquidityAmountCaCoin)
	res, err = handler(ctx, &msg)
	require.NoError(t, err)
	require.NotNil(t, res)
}
...
func TestRemoveLiquidity(t *testing.T) {
	...
	err := sojaapp.AddCoinsToAccount(clptypes.ModuleName, app.BankKeeper, ctx, signer, sdk.NewCoins(externalCoin, nativeCoin))
	require.NoError(t, err)
	err = sojaapp.AddCoinsToAccount(clptypes.ModuleName, app.BankKeeper, ctx, newLP, sdk.NewCoins(externalCoin, nativeCoin))
	require.NoError(t, err)
...
func TestSwap(t *testing.T) {
	...
	err := sojaapp.AddCoinsToAccount(clptypes.ModuleName, app.BankKeeper, ctx, signer, sdk.NewCoins(externalCoin1, nativeCoin))
	require.NoError(t, err)
	err = sojaapp.AddCoinsToAccount(clptypes.ModuleName, app.BankKeeper, ctx, signer, sdk.NewCoins(externalCoin2))
	require.NoError(t, err)
...
func UnlockAllliquidity(app *sojaapp.SojahubApp, ctx sdk.Context, asset clptypes.Asset, lp sdk.AccAddress, t *testing.T) {
