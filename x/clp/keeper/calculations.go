...
func CalcFuryValue(furyAmount sdk.Uint, price sdk.Dec) sdk.Uint {
	value := price.Mul(sdk.NewDecFromBigInt(furyAmount.BigInt()))
...
func CalcFurySpotPrice(pool *types.Pool, pmtpCurrentRunningRate sdk.Dec) (sdk.Dec, error) {
	furyBal, externalAssetBal := pool.ExtractDebt(pool.NativeAssetBalance, pool.ExternalAssetBalance, false)

	furyBalance := sdk.NewDecFromBigInt(furyBal.BigInt())
	if furyBalance.Equal(sdk.ZeroDec()) {
		return sdk.ZeroDec(), types.ErrInValidAmount
	}
	externalAssetBalance := sdk.NewDecFromBigInt(externalAssetBal.BigInt())
	unadjusted := externalAssetBalance.Quo(furyBalance)
...
func CalculateWithdrawalFuryValue(
	...
	X, Y, toFury, _ := pool.ExtractValues(to)

	X, Y = pool.ExtractDebt(X, Y, toFury)

	value, _ := CalcSwapResult(toFury, X, sentAmount, Y, pmtpCurrentRunningRate, swapFeeRate)
...
func SwapOne(from types.Asset,
	...
	Xincl, Yincl = pool.ExtractDebt(X, Y, toFury)

	priceImpact := calcPriceImpact(Xincl, sentAmount)
	swapResult, liquidityFee := CalcSwapResult(toFury, Xincl, sentAmount, Yincl, pmtpCurrentRunningRate, swapFeeRate)
	...
	pool.UpdateBalances(toFury, X, sentAmount, Y, swapResult)
...
func GetSwapFee(sentAmount sdk.Uint,
	...
	swapResult, _ := CalcSwapResult(toFury, X, sentAmount, Y, pmtpCurrentRunningRate, swapFeeRate)
