...
func (k Keeper) DistributeDepthRewards(ctx sdk.Context, blockDistribution sdk.Uint, period *types.RewardPeriod, pools []*types.Pool) error {
	...
	moduleBalancePreMinting := k.GetModuleFury(ctx)
	...
	poolFuryMap := make(PoolFuryMap)
	lpFuryMap := make(LpFuryMap)
	...
	for _, e := range tuples {
		if shouldDistribute {
			...
			CollectRewards(ctx, e.Pool, e.Reward, e.Pool.PoolUnits, lpsFiltered, poolFuryMap, lpFuryMap, lpPoolMap)
		} else {
			k.addRewardsToPool(ctx, e.Pool, e.Reward)
		}
	}
	...
	if shouldDistribute {
		poolFuryMapSum := sdk.ZeroUint()
		for _, fury := range poolFuryMap {
			poolFuryMapSum = poolFuryMapSum.Add(fury)
		}

		if !coinsToMint.Equal(poolFuryMapSum) {
			k.Logger(ctx).Info(fmt.Sprintln("coinsToMint", coinsToMint.String(), " != poolFuryMapSum", poolFuryMapSum.String()))
		}

		// this updates poolFuryMap in case coin tranfers fails
		k.TransferRewards(ctx, poolFuryMap, lpFuryMap, lpPoolMap)

		for pool, fury := range poolFuryMap {
			if fury.Equal(sdk.ZeroUint()) {
				continue
			}

			pool.RewardPeriodNativeDistributed = pool.RewardPeriodNativeDistributed.Add(fury)
		...
		moduleBalancePostTransfer := k.GetModuleFury(ctx)
		diff := moduleBalancePostTransfer.Sub(moduleBalancePreMinting).Amount // post is always >= pre

		if !diff.IsZero() {
			k.BurnFury(ctx, diff) // nolint:errcheck
		}
		coinsMinted := sdk.NewIntFromBigInt(coinsToMint.BigInt()).Sub(diff)
		fireRewardsEvent(ctx, "rewards/distribution", coinsMinted, PoolFuryMapToLPPools(poolFuryMap))
	} else {
...
func (k Keeper) TransferRewards(ctx sdk.Context, poolFuryMap PoolFuryMap, lpFuryMap LpFuryMap, lpPoolMap LpPoolMap) {
	k.TransferProviderDistributionGeneric(ctx, poolFuryMap, lpFuryMap, lpPoolMap, "rewards/distribution_error", "rewards/distribution")
}
...
func CollectRewards(ctx sdk.Context, pool *types.Pool, poolDistribution sdk.Uint, poolUnits sdk.Uint, lps []ValidLiquidityProvider, poolFuryMap PoolFuryMap, lpFuryMap LpFuryMap, lpPoolMap LpPoolMap) {
	furyToDistribute := CollectProviderDistribution(ctx, pool, sdk.NewDecFromBigInt(poolDistribution.BigInt()), sdk.NewDec(1), poolUnits, lps, lpFuryMap, lpPoolMap)
