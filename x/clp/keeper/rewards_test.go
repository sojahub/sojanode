...
import (
	...
	"github.com/Sojahub/sojanode/x/clp/keeper"
...
func TestEndBlock(t *testing.T) {
	...
	err := app.BankKeeper.MintCoins(ctx, types.ModuleName, sdk.NewCoins(
		sdk.NewCoin("fury", sdk.NewInt(3000)),
	...
	require.NoError(t, err)
	startingSupply := app.BankKeeper.GetSupply(ctx, "fury")
	...
	periodOneSupply := app.BankKeeper.GetSupply(ctx, "fury")
	...
	//supplyCheck := app.BankKeeper.GetSupply(ctx, "fury")
...
func TestKeeper_RewardsDistribution(t *testing.T) {
	...
	moduleBalance1 := app.ClpKeeper.GetModuleFury(ctx)
	...
	moduleBalance2 := app.ClpKeeper.GetModuleFury(ctx)
...
func TestKeeper_RewardsDistributionFailure(t *testing.T) {
	sojaapp.SetConfig(false) // needed for GenerateAddress to generate a proper address
	...
	moduleBalance := app.ClpKeeper.GetModuleFury(ctx)
