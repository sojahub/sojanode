...
func TestKeeper_CheckBalances(t *testing.T) {
	...
	const address = "soja1syavy2npfyt9tcncdtsdzf7kny9lh777yqc2nd"
...
func TestKeeper_ExtractValuesFromPool(t *testing.T) {
	...
	err := sojaapp.AddCoinsToAccount(types.ModuleName, app.BankKeeper, ctx, signer, sdk.NewCoins(externalCoin, nativeCoin))
	...
	X, Y, toFury, from := pool.ExtractValues(asset)

	assert.Equal(t, X, sdk.NewUint(998))
	assert.Equal(t, Y, sdk.NewUint(998))
	assert.Equal(t, toFury, false)
...
func TestKeeper_CalcSwapResult(t *testing.T) {
	testcases := []struct {
		...
		{
			name:                   "real world amounts, buy fury",
			toFury:                true,
		...
		{
			name:                   "real world amounts, sell fury",
			toFury:                false,
			X:                      sdk.NewUint(1999800619938006200),
			x:                      sdk.NewUint(200000000000000),
			Y:                      sdk.NewUint(2000200000000000000),
	...
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			y, fee := clpkeeper.CalcSwapResult(tc.toFury, tc.X, tc.x, tc.Y, tc.pmtpCurrentRunningRate, tc.swapFeeRate)
...
func TestKeeper_CalcFurySpotPrice(t *testing.T) {
	testcases := []struct {
		name                          string
		furyBalance, externalBalance sdk.Uint
		pmtpCurrentRunningRate        sdk.Dec
		...
		{
			name:                   "success small",
			furyBalance:           sdk.NewUint(1000000000123),
		...
		{
			name:                   "success",
			furyBalance:           sdk.NewUint(1000),
			externalBalance:        sdk.NewUint(2000),
			pmtpCurrentRunningRate: sdk.MustNewDecFromStr("1.4"),
			expectedPrice:          sdk.MustNewDecFromStr("4.8"),
		},
		{
			name:                   "fail - fury balance zero",
			furyBalance:           sdk.NewUint(0),
			externalBalance:        sdk.NewUint(2000),
			pmtpCurrentRunningRate: sdk.MustNewDecFromStr("1.4"),
	...
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			pool := types.Pool{
				NativeAssetBalance:   tc.furyBalance,
			...
			calcPrice, err := clpkeeper.CalcFurySpotPrice(&pool, tc.pmtpCurrentRunningRate)
...
func TestKeeper_CalcFuryValue(t *testing.T) {
	testcases := []struct {
		name          string
		furyAmount   sdk.Uint
		price         sdk.Dec
	...
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			furyValue := clpkeeper.CalcFuryValue(tc.furyAmount, tc.price)
			require.Equal(t, tc.expectedValue.String(), furyValue.String())
		})
	}
}
