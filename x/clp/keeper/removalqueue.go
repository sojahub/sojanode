...
func (k Keeper) QueueRemoval(ctx sdk.Context, msg *types.MsgRemoveLiquidity, furyValue sdk.Uint) {
	...
	queue.Count++
	queue.Id++
	queue.TotalValue = queue.TotalValue.Add(furyValue)
	if queue.Count == 1 {
...
func (k Keeper) ProcessRemovalQueue(ctx sdk.Context, msg *types.MsgAddLiquidity, unitsToDistribute sdk.Uint) {
	...
	for ; it.Valid(); it.Next() {
		...
		if msg.ExternalAsset.Equals(*request.Msg.ExternalAsset) {
			...
			_, _, totalFuryValue, err := k.ProcessRemoveLiquidityMsg(ctx, &types.MsgRemoveLiquidity{
			...
			k.SetProcessedRemovalRequest(ctx, request, withdrawWBasisPoints, totalFuryValue)
		}
	}
}
...
func (k Keeper) SetProcessedRemovalRequest(ctx sdk.Context, request types.RemovalRequest, pointsProcessed sdk.Int, furyRemoved sdk.Uint) {
	...
	queue.TotalValue = queue.TotalValue.Sub(furyRemoved)
	...
	emitProcessedRemovalRequest(ctx, &request, pointsProcessed, furyRemoved)
}
