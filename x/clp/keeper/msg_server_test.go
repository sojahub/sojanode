...
import (
	...
	admintest "github.com/Sojahub/sojanode/x/admin/test"
...
func TestMsgServer_DecommissionPool(t *testing.T) {
	testcases := []struct {
		...
		},
		{
			...
			poolUnits:           sdk.NewUint(1000),
			msg: &types.MsgDecommissionPool{
				Signer: "soja1jv65s3grqf6v6jl3dp4t6c9t9rk99cd8zzt2x5",
				Symbol: "eth",
			},
...
func TestMsgServer_Swap(t *testing.T) {
	testcases := []struct {
	...
	}{
		{
			...
			errString: errors.New("Token not supported by sojahub"),
		},
		{
...
func TestMsgServer_AddProviderDistribution(t *testing.T) {
	admin := "soja1gy2ne7m62uer4h5s4e7xlfq7aeem5zpwx6nu9q"
	nonAdmin := "soja1gy2ne7m62uer4h5s4e7xlfq7aeem5zpwx6nu9r"
	ctx, app := test.CreateTestAppClpFromGenesis(false, func(app *sojaapp.SojahubApp, genesisState sojaapp.GenesisState) sojaapp.GenesisState {
