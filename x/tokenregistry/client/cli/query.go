...
import (
	...
	"github.com/Sojahub/sojanode/x/tokenregistry/types"
	whitelistutils "github.com/Sojahub/sojanode/x/tokenregistry/utils"
)
...
func GetCmdGenerateEntry() *cobra.Command {
	...
	cmd.Flags().String(flagDenom, "",
		"The IBC hash / denom  stored on sojahub - to generate this hash for IBC token, leave blank and specify base_denom and ibc_channel_id")
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, entry := range registry.Entries {
				...
				if entry.Decimals > 10 {
					...
					} else if types.StringCompare(entry.Denom, "fury") {
						conversionDenom = "xfury"
					}
