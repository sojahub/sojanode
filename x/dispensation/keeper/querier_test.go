...
import (
	...
	dispensationkeeper "github.com/Sojahub/sojanode/x/dispensation/keeper"
...
func TestNewLegacyQuerier(t *testing.T) {
	...
	keeper := sojaapp.DispensationKeeper
	...
	qp, errRes := sojaapp.LegacyAmino().MarshalJSON(&queryRecName)
	...
	err = sojaapp.LegacyAmino().UnmarshalJSON(res, &dr)
	assert.Error(t, err)

}

func GenerateQueryData(app *app.SojahubApp, ctx sdk.Context, name string, outList []bank.Output) {
...
func TestQueryRecordsName(t *testing.T) {
	...
	GenerateQueryData(sojaapp, ctx, name, outList)
	keeper := sojaapp.DispensationKeeper
	querier := dispensationkeeper.NewLegacyQuerier(keeper)
	queryRecName := types.QueryRecordsByDistributionNameRequest{
		DistributionName: name,
...
func TestQueryRecordsAddr(t *testing.T) {
	...
	qp, errRes := sojaapp.LegacyAmino().MarshalJSON(&quereyRecName)
	require.NoError(t, errRes)
	query.Path = ""
	query.Data = qp
