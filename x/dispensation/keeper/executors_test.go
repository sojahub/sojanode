...
func createInput(t *testing.T, filename string) {
	in, err := sdk.AccAddressFromBech32("did:fury:s1syavy2npfyt9tcncdtsdzf7kny9lh777p07psd")
	assert.NoError(t, err)
	out, err := sdk.AccAddressFromBech32("did:fury:s1l7hypmqk2yc334vc6vmdwzp5sdefygj2ad93p5")
	assert.NoError(t, err)
...
func TestKeeper_AccumulateDrops(t *testing.T) {
	...
	for _, in := range inputList {
		...
		assert.NoError(t, err)
		err = sojaapp.AddCoinsToAccount(types.ModuleName, app.BankKeeper, ctx, address, in.Coins)
		assert.NoError(t, err)
	}
}
...
func TestKeeper_DistributeDrops_Fail(t *testing.T) {
	...
	err = sojaapp.AddCoinsToAccount(types.ModuleName, app.BankKeeper, ctx, dispensationCreator, totalCoins)
	assert.NoError(t, err)
...
func TestKeeper_CreateAndDistributeDrops(t *testing.T) {
	...
	for i := 0; i < len(outputList); i++ {
		...
		assert.True(t, recordsLM.DistributionRecords[i].Coins.AmountOf("fury").Equal(doubleOutputAmount) ||
