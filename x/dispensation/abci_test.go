...
import (
	types2 "github.com/Sojahub/sojanode/x/clp/types"
...
func Test_BeginBlocker(t *testing.T) {
	...
	fury := types2.GetSettlementAsset().Symbol
	...
	assert.Equal(t, sdk.ZeroInt(), app.DispensationKeeper.GetBankKeeper().GetBalance(ctx, ecoPoolAddress, fury).Amount)
	...
	for i := int64(0); i < expectedBlocks; i++ {
		...
		if i < expectedBlocks-1 {
			assert.Equal(t, expectedMintAmount, app.DispensationKeeper.GetBankKeeper().GetBalance(ctx, ecoPoolAddress, fury).Amount)
			// Assertion for last block
		} else {
			assert.Equal(t, totalMintAmount, app.DispensationKeeper.GetBankKeeper().GetBalance(ctx, ecoPoolAddress, fury).Amount)
		}
	...
	dispensation.BeginBlocker(ctx, app.DispensationKeeper)
	assert.Equal(t, app.DispensationKeeper.GetBankKeeper().GetBalance(ctx, ecoPoolAddress, fury).Amount, totalMintAmount)
