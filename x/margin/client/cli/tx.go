...
func GetUpdateFuryCollateralCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "update-fury-collateral",
		Short: "Enable or disable opening margin positions with fury collateral.",
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
			if err != nil {
				return err
			...
			furyCollateralEnabled := viper.GetBool("enabled")

			msg := types.MsgUpdateFuryCollateral{
				Signer:                 signer.String(),
				FuryCollateralEnabled: furyCollateralEnabled,
			}
	...
	cmd.Flags().Bool("enabled", true, "fury collateral enabled for margin [bool]")
