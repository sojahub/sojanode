...
func (k msgServer) OpenLong(ctx sdk.Context, msg *types.MsgOpen) (*types.MTP, error) {
	...
	if !k.IsFuryCollateralEnabled(ctx) && types.StringCompare(msg.CollateralAsset, nativeAsset) {
		return nil, sdkerrors.Wrap(types.ErrFuryAsCollateralNotAllowed, nativeAsset)
	}
...
func (k msgServer) UpdateFuryCollateral(goCtx context.Context, msg *types.MsgUpdateFuryCollateral) (*types.MsgUpdateFuryCollateralResponse, error) {
	ctx := sdk.UnwrapSDKContext(goCtx)
	signer, err := sdk.AccAddressFromBech32(msg.Signer)
	if err != nil {
		return nil, err
	...
	params := k.GetParams(ctx)
	params.FuryCollateralEnabled = msg.FuryCollateralEnabled
	k.SetParams(ctx, &params)

	return &types.MsgUpdateFuryCollateralResponse{}, nil
}
