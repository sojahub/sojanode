...
func TestKeeper_Open(t *testing.T) {
	table := []struct {
		...
		},
		{
			...
			poolAsset: "fury",
			token:     "somethingelse",
			errString: sdkerrors.Wrap(types.ErrMTPDisabled, "fury"),
		},
		{
		...
		},
		{
			...
			msgOpen: types.MsgOpen{
				Signer:           "soja1azpar20ck9lpys89r8x7zc8yu0qzgvtp48ng5v",
				CollateralAsset:  "xxx",
				CollateralAmount: sdk.NewUint(1000),
				BorrowAsset:      "xxx",
				Position:         types.Position_LONG,
	...
	for _, tt := range table {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.fundedAccount {
				...
				err = sojaapp.AddCoinsToAccount(types.ModuleName, app.BankKeeper, ctx, _signer, sdk.NewCoins(nativeCoin, externalCoin))
				require.Nil(t, err)
			} else {
	...
	for _, tt := range table {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.fundedAccount {
				...
				_signer := clptest.GenerateAddress(clptest.AddressKey1)
				address = _signer.String()
				err = sojaapp.AddCoinsToAccount(types.ModuleName, app.BankKeeper, ctx, _signer, sdk.NewCoins(nativeCoin))
				require.NoError(t, err)
...
func TestKeeper_ForceClose(t *testing.T) {
	table := []struct {
	...
	}{
		{
			...
			msgForceClose: types.MsgForceClose{
				Signer:     "soja1azpar20ck9lpys89r8x7zc8yu0qzgvtp48ng5v",
				MtpAddress: "soja1azpar20ck9lpys89r8x7zc8yu0qzgvtp48ng5v",
			},
			msgOpen: types.MsgOpen{
				CollateralAsset: "xxx",
				BorrowAsset:     "xxx",
	...
	for _, tt := range table {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx, app := test.CreateTestAppMarginFromGenesis(false, func(app *sojaapp.SojahubApp, genesisState sojaapp.GenesisState) sojaapp.GenesisState {
				...
				gs3 := &types.GenesisState{
					Params: &types.Params{
						...
						ForceCloseFundAddress:                    "soja1syavy2npfyt9tcncdtsdzf7kny9lh777yqc2nd",
						IncrementalInterestPaymentFundPercentage: sdk.NewDecWithPrec(1, 1),
						IncrementalInterestPaymentFundAddress:    "soja1syavy2npfyt9tcncdtsdzf7kny9lh777yqc2nd",
...
func TestKeeper_OpenClose(t *testing.T) {
	table := []struct {
	...
	}{
		{
			...
			// errString: errors.New("fury: using fury as collateral asset is not allowed"),
		},
	}
	...
	for _, tt := range table {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			err = sojaapp.AddCoinsToAccount(types.ModuleName, app.BankKeeper, ctx, signer, sdk.NewCoins(nativeCoin, externalCoin))
			require.Nil(t, err)
...
func TestKeeper_OpenThenClose(t *testing.T) {
	...
	ctx, app := test.CreateTestAppMarginFromGenesis(false, func(app *sojaapp.SojahubApp, genesisState sojaapp.GenesisState) sojaapp.GenesisState {
		...
		balances := []banktypes.Balance{
			{
				Address: "soja1pjm228rsgwqf23arkx7lm9ypkyma7mzr3y2n85",
				Coins: sdk.Coins{
