...
func TestKeeper_Borrow(t *testing.T) {
	...
	borrowTests := []struct {
		...
		},
		{
			...
			decimals:         18,
			to:               "fury",
			address:          "soja1azpar20ck9lpys89r8x7zc8yu0qzgvtp48ng5v",
			collateralAmount: sdk.NewUint(10000),
			custodyAmount:    sdk.NewUint(1000),
			leverage:         sdk.NewDec(1),
			health:           sdk.NewDec(1),
...
func TestWhitelist(t *testing.T) {
	...
	marginKeeper.WhitelistAddress(ctx, "soja123")
	is := marginKeeper.IsWhitelisted(ctx, "soja123")
	require.True(t, is)
	whitelist, _, err := marginKeeper.GetWhitelist(ctx, nil)
	require.NoError(t, err)
	require.Equal(t, []string{"soja123"}, whitelist)
}
