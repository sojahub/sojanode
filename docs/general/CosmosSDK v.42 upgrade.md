...
The Sojanode project is built with Cosmos SDK &amp; Tendermint engine. This document describes the most noticeable changes that have been made as part of the major update of the Sojanode codebase in order to to migrate from v0.39 to v0.42 of the Cosmos SDK. It aims to give context required to be able to interact with Sojanode after the upgrade, rather than doing a complete overview of modifications.
...
**sojanoded** and **sojanoded** have been combined into **sojanoded** , so all commands previously executed through sojanoded are now executed through **sojanoded**. Aside from the merge into sidnoded, command syntax remains largely unchanged. The changes to scripts in **scripts/demo** are minimal beyond the shift from sojanoded to sojanoded.

Syntax changes:

- **send** tx is now under the **bank** route rather than a root **tx** command

  - **Old:** sojanoded tx send [from\_key\_or\_address] [to\_address] [amount] [flags]
  - **New:** sojanoded tx bank send [from\_key\_or\_address] [to\_address] [amount] [flags]

Commands are no longer added in **cmd/sojanoded/main.go** , each route is now added in the following files:

- **cmd/sojanoded/cmd/root.go**
- **cmd/sojanoded/cmd/oracle.go**
- **cmd/sojanoded/cmd/migrate.go**
- **cmd/sojanoded/cmd/gentx.go**
- **cmd/sojanoded/cmd/genaccounts.go**
- **cmd/sojanoded/cmd/clpadmin.go**
...
- **proto/sojanode/module_name/v1/\*.proto** - define module-specific types as Protobuf messages/services.
...
- **proto/sojanode/clp/v1/types.proto** - **LiquidityProvider.liquidity_provider_address** type has been changed from **sdk.AccAddress** to **string**.
- **proto/sojanode/clp/v1/querier.proto** - **PoolRes.clp_module_address** type has been changed from **ClpModuleAddress** to **string**
- **proto/sojanode/clp/v1/querier.proto** - **LiquidityProviderRes.native_asset_balance** type has been changed from **NativeAssetBalance** to **string**
- **proto/sojanode/clp/v1/querier.proto** - **LiquidityProviderRes.external_asset_balance** type has been changed from **ExternalAssetBalance** to **string**.
