...
Sojanoded does not currently support asymmetric liquidity adds. This document proposes a procedure
...
```
Currently sojanoded blocks asymmetric adds. The following procedure is proposed to enable
asymmetric adds.
...
```bash
sojanoded query clp estimate-pool-share \
  --externalAmount=0 \
