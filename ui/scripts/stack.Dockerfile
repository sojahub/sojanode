...
COPY --from=build-go /go/bin/sojanoded /usr/bin/sojanoded
COPY --from=build-go /go/bin/sojagen /usr/bin/sojagen

# Required for ebrelayer
COPY --from=build-go /soja/cmd/ebrelayer /soja/cmd/ebrelayer
