...
export async function initRelayer(props) {
  // 0) retrieve sojahub props
  const candidateSojaChainProps = getChainProps({ chain: "sojahub" });
  ...
  const {
    ...
    p2pPortA = 26656,
    pprofPortA = 6060,
    homeA = `/tmp/localnet/config/${candidateSojaChainProps.chain}/${candidateSojaChainProps.chainId}`,
    rpcPortB = 36657,
    p2pPortB = 36656,
    pprofPortB = 7060,
  ...
  const sojaChainProps = {
    ...candidateSojaChainProps,
    rpcPort: rpcPortA,
    p2pPort: p2pPortA,
  ...
  await createRelayerRegistry({
    chainsProps: [sojaChainProps, otherChainProps],
    registryFrom,
  });
