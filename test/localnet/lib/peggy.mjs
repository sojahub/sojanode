...
export async function ibc(props) {
  const sojaChainProps = getChainProps({ chain: "sojahub" });
  ...
  const {
    ...
    ...props,
    chain: props.type === "issuer" ? props.chain : "sojahub",
...
`);
  ...
  const srcAddr = await getAddress({
    binary: type === "receiver" ? sojaChainProps.binary : props.binary,
    name,
  });
  const dstAddr = await getAddress({
    binary: type === "receiver" ? props.binary : sojaChainProps.binary,
    name,
  });
