...
const chainAProps = getChainProps({ chain: "sojahub", network: "testnet-1" });
...
test("ibc transfer between two chains", async () => {
  ...
  const channel =
    await $`ibc-setup channels --chain sojahub --home ${chainBHome}/relayer/ | grep ${connection} | cut -d ' ' -f 1`;
