...
from sojatool.common import project_dir, sojatool_logger
...
def run_localnet_hook():
    ...
    # rm -rf /tmp/localnet/config/sojahub/sojahub-testnet-1
    # mkdir -p /tmp/localnet/config/sojahub/sojahub-testnet-1
    # /tmp/localnet/bin/sojanoded init sojahub-testnet-1 --chain-id sojahub-testnet-1 --home /tmp/localnet/config/sojahub/sojahub-testnet-1
    # /tmp/localnet/bin/sojanoded keys add sojahub-validator --keyring-backend test --home /tmp/localnet/config/sojahub/sojahub-testnet-1
    # /tmp/localnet/bin/sojanoded keys add sojahub-source --keyring-backend test --home /tmp/localnet/config/sojahub/sojahub-testnet-1
    # /tmp/localnet/bin/sojanoded add-genesis-account sojahub-validator 10000000000000000000fury --keyring-backend test --home /tmp/localnet/config/sojahub/sojahub-testnet-1
    # /tmp/localnet/bin/sojanoded add-genesis-account sojahub-source 10000000000000000000fury --keyring-backend test --home /tmp/localnet/config/sojahub/sojahub-testnet-1
    ...
    #
    # if sojahub: ${binPath}/${binary} set-gen-denom-whitelist ${home}/config/denoms.json --home ${home}
    #
    ...
    localnet.start_all_chains()  # Runs sojanoded and gaiad
...
def download_ibc_binaries(cmd, chains_to_download=None, output_path=None):
    ...
    config = get_localnet_config(cmd)
    tmpdir = cmd.mktempdir()
    # We prefer to compile sojahub. Sentinel uses sourceUrl, but there is no Makefile.
    all_supported_chains = set(config.keys()).difference({"sojahub", "sentinel"})
