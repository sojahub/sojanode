...
# Test environment for testing the new Sojahub public SDK: https://docs.google.com/document/d/1MAlg-I0xMnUvbavAZdAN---WuqbyuRyKw-6Lfgfe130/edit
# IBC localnet tool: https://github.com/Sojahub/sojahub-deploy/commit/0424d2ec81f6f739a3a2cbc77feba1a6f297430d
...
chains = {
    ...
    "sojahub": {"binary": "sojanoded", "relayer": "ibc"},
}
...
# Generates a sojanoded key and stores it into test keyring. Returns the mnemonic that can be used to
# recreate it.
...
def run_tests_for_one_chain_in_one_direction(config, other_chain, direction_flag, number_of_iterations):
    # Assume we're always sending transactions between other_chain and sojahub.
    # from_chain is chain sending the assets, to_chain is the receiving chain.
    from_chain = "sojahub" if direction_flag else other_chain
    to_chain = other_chain if direction_flag else "sojahub"
    ...
    fees = config["fees"]
    gas = config["gas"]
    sojahub_proc = start_chain("sojahub")
    ...
    mnemonic = add_new_key_to_keyring("sojahub", from_account)
