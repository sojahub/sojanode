...
from sojatool import test_utils, run_env, cosmos, diagnostics, sojahub, frontend, test_utils2
from sojatool.run_env import Integrator, UIStackEnvironment, Peggy2Environment, IBCEnvironment, IntegrationTestsEnvironment
...
def main(argv):
    ...
    if what == "venv":
        ...
        if class_to_use == Peggy2Environment:
            ...
            hardhat_proc, sojanoded_proc, relayer0_proc, witness_procs = env.run()
            processes = [hardhat_proc, sojanoded_proc, relayer0_proc] + witness_procs
            ...
            processes = env.run()
            # TODO Cleanup:
            # - rm -rf test/integration/sojanoderelayerdb
            # - rm -rf networks/validators/localnet/$moniker/.sojanoded
        ...
        import sojatool.geth, sojatool.eth
        geth = sojatool.geth.Geth(cmd)
        datadir = os.path.join(os.environ["HOME"], ".sojatool-geth")
        datadir = None
        signer_addr, signer_private_key = sojatool.eth.web3_create_account()
        ...
        sojanoded = sojahub.Sojanoded(cmd, node=args.node)
        from_block = args.from_block if args.from_block is not None else 1
        to_block = args.to_block if args.to_block is not None else sojanoded.get_current_block()
        block_times = diagnostics.get_block_times(sojanoded, from_block, to_block)
