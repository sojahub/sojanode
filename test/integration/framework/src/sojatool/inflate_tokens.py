...
# The original script had a lot of problems as described in https://app.zenhub.com/workspaces/current-sprint---engineering-615a2e9fe2abd5001befc7f9/issues/sojahub/issues/719.
# See https://www.notion.so/sojahub/TEST-TOKEN-DISTRIBUTION-PROCESS-41ad0861560c4be58918838dbd292497

import json
import re
from typing import Any, Mapping, Iterable, Sequence

from sojatool import eth, test_utils, cosmos
...
class InflateTokens:
    ...
    def get_whitelisted_tokens(self) -> List[TokenDict]:
        ...
        for token_addr, value in whitelist.items():
            ...
            token = {
                ...
                "soja_denom": self.ctx.eth_symbol_to_soja_symbol(token_symbol),
            }
    ...
    def build_list_of_tokens_to_create(self, existing_tokens: Iterable[TokenDict], requested_tokens: Iterable[TokenDict]
        ...
        """
        This part deploys SojahubTestoken for every requested token that has not yet been deployed.
    ...
    def transfer_from_eth_to_sojanode(self, from_eth_addr, to_soja_addr, tokens_to_transfer, amount_in_tokens, amount_eth_gwei):
        soja_balances_before = self.ctx.get_sojahub_balance(to_soja_addr)
        sent_amounts = []
        pending_txs = []
        for token in tokens_to_transfer:
            ...
            token_sc = self.ctx.get_generic_erc20_sc(token_addr)
            amount = amount_in_tokens * 10**decimals
            pending_txs.extend(self.ctx.tx_approve_and_lock(token_sc, from_eth_addr, to_soja_addr, amount))
            sent_amounts.append([amount, token["soja_denom"]])
        if amount_eth_gwei > 0:
            amount = amount_eth_gwei * eth.GWEI
            pending_txs.append(self.ctx.tx_bridge_bank_lock_eth(from_eth_addr, to_soja_addr, amount))
        ...
        # Wait for intermediate_soja_account to receive all funds across the bridge
        ...
        self.ctx.sojanode.wait_for_balance_change(to_soja_addr, soja_balances_before, min_changes=sent_amounts,
            polling_time=5, timeout=0, change_timeout=self.wait_for_account_change_timeout)

    # Distributes from intermediate_soja_account to each individual account
    def distribute_tokens_to_wallets(self, from_soja_account, tokens_to_transfer, amount_in_tokens, target_soja_accounts, amount_eth_gwei):
        send_amounts = [[amount_in_tokens * 10**t["decimals"], t["soja_denom"]] for t in tokens_to_transfer]
        if amount_eth_gwei > 0:
            send_amounts.append([amount_eth_gwei * eth.GWEI, self.ctx.ceth_symbol])
        progress_total = len(target_soja_accounts) * len(send_amounts)
        progress_current = 0
        for soja_acct in target_soja_accounts:
            remaining = send_amounts
            while remaining:
                batch_size = len(remaining)
                if (self.ctx.sojanode.max_send_batch_size > 0) and (batch_size > self.ctx.sojanode.max_send_batch_size):
                    batch_size = self.ctx.sojanode.max_send_batch_size
                batch = remaining[:batch_size]
                remaining = remaining[batch_size:]
                soja_balance_before = self.ctx.get_sojahub_balance(soja_acct)
                self.ctx.send_from_sojahub_to_sojahub(from_soja_account, soja_acct, batch)
                self.ctx.sojanode.wait_for_balance_change(soja_acct, soja_balance_before, min_changes=batch,
    ...
    def transfer(self, requested_tokens: Sequence[TokenDict], token_amount: int,
        target_soja_accounts: Sequence[cosmos.Address], eth_amount_gwei: int
    ):
        """
        It goes like this:
        ...
        3. It will do a single transaction across the bridge to move all tokens from OPERATOR to soja_broker_account
        4. It will distribute tokens from soja_broker_account to each of given target accounts
        The soja_broker_account and OPERATOR can be any Sojahub and Ethereum accounts, we might want to use something
        ...
        n_accounts = len(target_soja_accounts)
        ...
        number_of_batches = 1 if self.ctx.sojanode.max_send_batch_size == 0 else (len(requested_tokens) + 1) // self.ctx.sojanode.max_send_batch_size + 1
        fund_fury = [5 * test_utils.sojanode_funds_for_transfer_peggy1 * n_accounts * number_of_batches, "fury"]
        log.debug("Estimated number of batches needed to transfer tokens from intermediate soja account to target soja wallet: {}".format(number_of_batches))
        log.debug("Estimated fury funding needed for intermediate account: {}".format(fund_fury))
        ether_faucet_account = self.ctx.operator
        soja_broker_account = self.ctx.create_sojahub_addr(fund_amounts=[fund_fury])
        ...
        log.info("Using soja_broker_account {}".format(soja_broker_account))

        # Check first that we have the key for FURY_SOURCE since the script uses it as an intermediate address
        keys = self.ctx.sojanode.keys_list()
        fury_source_key = zero_or_one([k for k in keys if k["address"] == soja_broker_account])
        assert fury_source_key is not None, "Need private key of broker account {} in sojanoded test keyring".format(soja_broker_account)
        ...
        if (self.max_ethereum_batch_size > 0) and (len(tokens_to_transfer) > self.max_ethereum_batch_size):
            log.debug(f"Transferring {len(tokens_to_transfer)} tokens from ethereum to sojandde in batches of {self.max_ethereum_batch_size}...")
            remaining = tokens_to_transfer
            while remaining:
                batch = remaining[:self.max_ethereum_batch_size]
                remaining = remaining[self.max_ethereum_batch_size:]
                self.transfer_from_eth_to_sojanode(eth_broker_account, soja_broker_account, batch, total_token_amount, 0)
                log.debug(f"Batch completed, {len(remaining)} tokens remaining")
            # Transfer ETH separately
            log.debug("Thansfering ETH from ethereum to sojanode...")
            self.transfer_from_eth_to_sojanode(eth_broker_account, soja_broker_account, [], 0, total_eth_amount_gwei)
        else:
            log.debug(f"Transferring {len(tokens_to_transfer)} tokens from ethereum to sojanode in single batch...")
            self.transfer_from_eth_to_sojanode(eth_broker_account, soja_broker_account, tokens_to_transfer, total_token_amount, total_eth_amount_gwei)
        self.distribute_tokens_to_wallets(soja_broker_account, tokens_to_transfer, token_amount, target_soja_accounts, eth_amount_gwei)
        ...
        log.info("To see newly minted tokens in UI, you need to edit 'scripts/ibc/tokenregistry/generate-erc20-jsons.sh' "
            ...
            "and commit the results in the sojahub-devnet-1 folder. @tim will pick up the PR and register it on "
    ...
    def transfer_eth(self, from_eth_addr: eth.Address, amount_gwei: int, target_soja_accounts: Iterable[cosmos.Address]):
        pending_txs = []
        for soja_acct in target_soja_accounts:
            txrcpt = self.ctx.tx_bridge_bank_lock_eth(from_eth_addr, soja_acct, amount_gwei * eth.GWEI)
            pending_txs.append(txrcpt)
        self.wait_for_all(pending_txs)


def run(*args):
    # This script should be run with SOJATOOL_ENV_FILE set to a file containing definitions for OPERATOR_ADDRESS,
