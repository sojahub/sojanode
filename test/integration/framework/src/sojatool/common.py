...
# TODO Move to sojahub.py
# TODO Refactoring in progress. This should be moved to sojahub.py and only used for gas (float amount) + renamed
def soja_format_amount(amount: Union[int, float], denom: str) -> str:
...
def sojatool_logger(name: Optional[str] = None):
    # Shortening "sojatool.eth" to "eth" results in a name clash with "eth" dependencies for which we want to disable logging...
    # if name is not None:
...
__log = sojatool_logger(__name__)
