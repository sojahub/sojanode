...
import sojatool.eth
from sojatool import eth, cosmos, command
from sojatool.hardhat import Hardhat, default_accounts as hardhat_default_accounts
from sojatool.geth import Geth
from sojatool.truffle import Ganache
from sojatool.localnet import Localnet
from sojatool.command import Command
from sojatool.sojahub import Sojagen, Sojanoded, Ebrelayer, sojahub_denom_hash, FURY
from sojatool.project import Project, killall, force_kill_processes
from sojatool.common import *


log = sojatool_logger(__name__)
...
class Integrator(Ganache, Command):
    ...
    # Peggy1 only
    def sojahub_init_integration(self, sojanode, validator_moniker, validator_mnemonic, denom_whitelist_file):
        # now we have to add the validator key to the test keyring so the tests can send fury from validator1
        sojanode0 = Sojanoded(self)
        sojanode0.keys_add(validator_moniker, validator_mnemonic)
        valoper = sojanode.keys_show(validator_moniker, bech="val")[0]["address"]
        assert valoper == sojanode0.get_val_address(validator_moniker)  # This does not use "home"; if it the assertion holds it could be grouped with sojahub_init_peggy

        # This was deleted in commit f00242302dd226bc9c3060fb78b3de771e3ff429 from sojahub_start_daemon.sh because
        # it was not working. But we assume that we want to keep it.
        sojanode.sojanoded_exec(["add-genesis-validators", valoper] + sojanode._home_args())

        # Add sojanodeadmin to ~/.sojanoded
        sojanode0 = Sojanoded(self)
        adminuser_addr = sojanode0.keys_add("sojanodeadmin")["address"]
        tokens = {FURY: 10 ** 28}
        # Original from peggy:
        # self.cmd.execst(["sojanoded", "add-genesis-account", sojanoded_admin_address, "100000000000000000000fury", "--home", sojanoded_home])
        sojanode.add_genesis_account(adminuser_addr, tokens)
        sojanode.set_genesis_oracle_admin(adminuser_addr)
        sojanode.set_gen_denom_whitelist(denom_whitelist_file)

        return adminuser_addr

    def sojanoded_peggy2_init_validator(self, sojanode, validator_moniker, validator_mnemonic, evm_network_descriptor, validator_power):
        # Add validator key to test keyring
        # This effectively copies key for validator_moniker from what sojagen creates in /tmp/sojanodedNetwork/validators
        # to ~/.sojanoded (note absence of explicit sojanoded_home, therefore it's ~/.sojanoded)
        sojanode0 = Sojanoded(self)
        ...
        valoper = sojanode.get_val_address(validator_moniker)

        # Add genesis validator
        sojanode.add_genesis_validators_peggy(evm_network_descriptor, valoper, validator_power)

    # @TODO Move to Sojagen class
    def sojagen_create_network(self, chain_id: str, validator_count: int, networks_dir: str, network_definition_file: str,
        seed_ip_address: str, mint_amount: Optional[cosmos.Balance] = None
    ):
        # Old call (no longer works either):
        # sojagen network create localnet 1 /mnt/shared/work/projects/soja/sojanode/local-tmp/my/deploy/rake/../networks \
        #     192.168.1.2 /mnt/shared/work/projects/soja/sojanode/local-tmp/my/deploy/rake/../networks/network-definition.yml \
        #     --keyring-backend file
        # self.cmd.execst(["sojagen", "network", "create", "localnet", str(validator_count), networks_dir, seed_ip_address,
        ...
        args = ["sojagen", "network", "create", chain_id, str(validator_count), networks_dir, seed_ip_address,
            network_definition_file, "--keyring-backend", "test"] + \
            (["--mint-amount", cosmos.balance_format(mint_amount)] if mint_amount else [])
        self.execst(args)

    # def wait_for_soja_account(self, netdef_json, validator1_address):
    #     # TODO Replace with test_utilities.wait_for_soja_account / wait_for_soja_account_up
    #     return self.execst(["python3", os.path.join(self.project.test_integration_dir, "src/py/wait_for_soja_account.py"),
    #         netdef_json, validator1_address], env={"USER1ADDR": "nothing"})

    def wait_for_soja_account_up(self, address: cosmos.Address, tcp_url: str = None, timeout: int = 90):
        # TODO Move to Sojanoded class
        ...
        #     time.sleep(1)
        # self.wait_for_soja_account(netdef_json, validator1_address)
        ...
        args = ["sojanoded", "query", "account", address] + \
        ...
        while True:
            try:
                ...
                if last_exception is None:
                    log.debug(f"Waiting for soja account {address}...")
                if time.time() - start_time > timeout:
                    message = "Timeout expired while waiting for soja account {} to become available".format(address)
...
class UIStackEnvironment:
    def __init__(self, cmd):
        ...
        self.sojanoded_path = self.cmd.get_user_home(".sojanoded")
        self.sojanode = Sojanoded(cmd)
    ...
    def stack_save_snapshot(self):
        ...
        # ui/scripts/stack-launch.sh -> ui/scripts/_soja-build.sh -> ui/chains/soja/build.sh
        # killall sojanoded
        # rm $(which sojanoded)
        self.cmd.rmdir(self.sojanoded_path)
        ...
        sojanode = Sojanoded(self.cmd)
        # ui/scripts/stack-launch.sh -> ui/scripts/_soja.sh -> ui/chains/soja/launch.sh
        sojanode.sojanoded_init("test", self.chain_id)
        self.cmd.copy_file(project_dir("ui/chains/soja/app.toml"), os.path.join(self.sojanoded_path, "config/app.toml"))
        log.info(f"Generating deterministic account - {self.shadowfiend_name}...")
        shadowfiend_account = self.cmd.sojanoded_keys_add(self.shadowfiend_name, self.shadowfiend_mnemonic)
        log.info(f"Generating deterministic account - {self.akasha_name}...")
        akasha_account = self.sojanode.keys_add(self.akasha_name, self.akasha_mnemonic)
        log.info(f"Generating deterministic account - {self.juniper_name}...")
        juniper_account = self.cmd.sojanoded_keys_add(self.juniper_name, self.juniper_mnemonic)
        ...
        assert shadowfiend_address == self.sojanode.keys_show(self.shadowfiend_name)[0]["address"]
        assert akasha_address == self.sojanode.keys_show(self.akasha_name)[0]["address"]
        assert juniper_address == self.sojanode.keys_show(self.juniper_name)[0]["address"]

        tokens_shadowfiend = [[10**29, "fury"], [10**29, "catk"], [10**29, "cbtk"], [10**29, "ceth"], [10**29, "cusdc"], [10**29, "clink"], [10**26, "stake"]]
        tokens_akasha = [[10**29, "fury"], [10**29, "catk"], [10**29, "cbtk"], [10**29, "ceth"], [10**29, "cusdc"], [10**29, "clink"], [10**26, "stake"]]
        tokens_juniper = [[10**22, "fury"], [10**22, "cusdc"], [10**20, "clink"], [10**20, "ceth"]]
        sojanode.add_genesis_account(shadowfiend_address, tokens_shadowfiend)
        sojanode.add_genesis_account(akasha_address, tokens_akasha)
        sojanode.add_genesis_account(juniper_address, tokens_juniper)

        shadowfiend_address_bech_val = sojanode.keys_show(self.shadowfiend_name, bech="val")[0]["address"]
        self.cmd.sojanoded_add_genesis_validators(shadowfiend_address_bech_val)

        amount = soja_format_amount(10**24, "stake")
        self.cmd.execst(["sojanoded", "gentx", self.shadowfiend_name, amount, f"--chain-id={self.chain_id}",
            f"--keyring-backend={self.keyring_backend}"])

        log.info("Collecting genesis txs...")
        self.cmd.execst(["sojanoded", "collect-gentxs"])
        log.info("Validating genesis file...")
        self.cmd.execst(["sojanoded", "validate-genesis"])

        log.info("Starting test chain...")
        sojanoded_proc = self.cmd.sojanoded_start(minimum_gas_prices=(0.5, FURY))  # TODO sojanoded_home=???

        # sojanoded must be up before continuing
        self.cmd.soja_wait_up("localhost", 1317)
        ...
        # ui/scripts/_migrate.sh -> ui/chains/soja/migrate.sh
        # Original scripts say "if we don't sleep there are issues"
        time.sleep(10)
        log.info("Creating liquidity pool from catk:fury...")
        sojanode.tx_clp_create_pool(self.chain_id, self.keyring_backend, "akasha", "catk", [10**5, FURY], 10**25, 10**25)
        time.sleep(5)
        log.info("Creating liquidity pool from cbtk:fury...")
        sojanode.tx_clp_create_pool(self.chain_id, self.keyring_backend, "akasha", "cbtk", [10**5, FURY], 10**25, 10**25)
        # should now be able to swap from catk:cbtk
        time.sleep(5)
        log.info("Creating liquidity pool from ceth:fury...")
        sojanode.tx_clp_create_pool(self.chain_id, self.keyring_backend, "akasha", "ceth", [10**5, FURY], 10**25, 83*10**20)
        # should now be able to swap from x:ceth
        time.sleep(5)
        log.info("Creating liquidity pool from cusdc:fury...")
        sojanode.tx_clp_create_pool(self.chain_id, self.keyring_backend, "akasha", "cusdc", [10**5, FURY], 10**25, 10**25)
        time.sleep(5)
        log.info("Creating liquidity pool from clink:fury...")
        sojanode.tx_clp_create_pool(self.chain_id, self.keyring_backend, "akasha", "clink", [10**5, FURY], 10**25, 588235*10**18)
        time.sleep(5)
        log.info("Creating liquidity pool from ctest:fury...")
        sojanode.tx_clp_create_pool(self.chain_id, self.keyring_backend, "akasha", "ctest", [10**5, FURY], 10**25, 10**13)
        ...
        # At this point we have 3 running processes - ganache_proc, sojanoded_proc and ebrelayer_proc
        # await soja-node-up and migrate-complete
        ...
        # ui/chains/soja/snapshot.sh:
        self.cmd.tar_create(self.sojanoded_path, os.path.join(snapshots_dir, "soja.tar.gz"))
    ...
    def stack_push(self):
        ...
        image_root = "ghcr.io/sojahub/sojanode/ui-stack"
...
class IntegrationTestsEnvironment:
    def __init__(self, cmd):
        ...
        # set_persistant_env_var SOJAHUB_BIN $BASEDIR/cmd $envexportfile
        ...
        # set_persistant_env_var CONTAINER_NAME integration_sojanode1_1 $envexportfile
        ...
        self.network_name = "develop"
        self.network_id = 5777
        self.peruser_storage_dir = self.cmd.get_user_home(".sojanode-integration")
        self.state_vars = {}
    ...
    def run(self):
        ...
        log_dir = self.cmd.tmpdir("sojanode")
        self.cmd.mkdir(log_dir)
        ganache_log_file = open(os.path.join(log_dir, "ganache.log"), "w")  # TODO close
        sojanoded_log_file = open(os.path.join(log_dir, "sojanoded.log"), "w")  # TODO close
        ...
        # # TODO This should be last (after return from setup_sojahub.sh)
        # burn_limits = [
        #     [eth.NULL_ADDRESS, 31*10**18],
        ...
        # test/integration/setup_sojahub.sh:
        ...
        # New:
        # sojagen network create localnet 1 $NETWORKDIR 192.168.1.2 $NETWORKDIR/network-definition.yml --keyring-backend test \
        #     --mint-amount 999999000000000000000000000fury,1370000000000000000ibc/FEEDFACEFEEDFACEFEEDFACEFEEDFACEFEEDFACEFEEDFACEFEEDFACEFEEDFACE
        chain_id = "localnet"
        validator_count = 1
        network_definition_file = os.path.join(networks_dir, "network-definition.yml")
        seed_ip_address = "192.168.1.2"

        self.cmd.sojagen_create_network(chain_id, validator_count, networks_dir, network_definition_file, seed_ip_address, mint_amount=self.mint_amount)
        ...
        sojanoded_home = os.path.join(chaindir, ".sojanoded")
        denom_whitelist_file = os.path.join(self.test_integration_dir, "whitelisted-denoms.json")
        # SOJANODED_LOG=$datadir/logs/sojanoded.log

        sojanode = Sojanoded(self.cmd, home=sojanoded_home)

        adminuser_addr = self.cmd.sojahub_init_integration(sojanode, validator1_moniker, validator1_mnemonic,
            denom_whitelist_file)

        # Start sojanoded
        sojanoded_proc = sojanode.sojanoded_start(tcp_url=self.tcp_url, minimum_gas_prices=(0.5, FURY),
            log_file=sojanoded_log_file, trace=True)

        # TODO: wait for sojanoded to come up before continuing
        # in sojahub_start_daemon.sh: "sleep 10"
        # in sojahub_run_ebrelayer.sh (also run_ebrelayer here) we already wait for connection to port 26657 and soja account validator1_addr

        # Removed
        # # TODO Process exits immediately with returncode 1
        # # TODO Why does it not stop start-integration-env.sh?
        # # rest_server_proc = self.cmd.popen(["sojanoded", "rest-server", "--laddr", "tcp://0.0.0.0:1317"])  # TODO cwd

        # test/integration/sojahub_start_ebrelayer.sh -> test/integration/sojahub_run_ebrelayer.sh
        # This script is also called from tests

        relayer_db_path = os.path.join(self.test_integration_dir, "sojahubrelayerdb")
        ...
        self.state_vars = {
            ...
            "EBRELAYER_ETHEREUM_PRIVATE_KEY": ebrelayer_ethereum_private_key,  # Needed by sojahub_run_ebrelayer.sh
            ...
            "SOJAHUB_ADMIN_ACCOUNT": adminuser_addr,  # Needed by test_peggy_fees.py (via conftest.py)
            "EBRELAYER_DB": relayer_db_path,  # Created by sojahub_run_ebrelayer.sh, does not appear to be used anywhere at the moment
        }
        self.project.write_vagrantenv_sh(self.state_vars, self.data_dir, self.ethereum_websocket_address, self.chainnet)

        log.debug("Admin account address (fury source): {}".format(adminuser_addr))

        from sojatool import localnet
        localnet.run_localnet_hook()

        return ganache_proc, sojanoded_proc, ebrelayer_proc

    def remove_and_add_sojanoded_keys(self, moniker, mnemonic):
        ...
        # res = self.cmd.execst(["sojanoded", "keys", "delete", moniker, "--keyring-backend", "test"], stdin=["y"])
        sojanode = Sojanoded(self.cmd)
        sojanode.keys_delete(moniker)
        sojanode.keys_add(moniker, mnemonic)
    ...
    def run_ebrelayer(self, netdef_json, validator1_address, validator_moniker, validator_mnemonic,
        ...
        while not self.cmd.tcp_probe_connect("localhost", 26657):
            time.sleep(1)
        # self.cmd.wait_for_soja_account(netdef_json, validator1_address)
        self.cmd.wait_for_soja_account_up(validator1_address)
        time.sleep(10)
        self.remove_and_add_sojanoded_keys(validator_moniker, validator_mnemonic)  # Creates ~/.sojanoded/keyring-tests/xxxx.address
    ...
    def create_snapshot(self, snapshot_name):
        ...
        self.cmd.tar_create(self.state_vars["EBRELAYER_DB"], os.path.join(named_snapshot_dir, "sojahubrelayerdb.tar.gz"))
        ...
        self.cmd.tar_create(self.cmd.get_user_home(".sojanoded"), os.path.join(named_snapshot_dir, "sojanoded.tar.gz"))
    ...
    def restore_snapshot(self, snapshot_name):
        ...
        assert os.path.realpath(relayer_db_path) == os.path.realpath(os.path.join(self.test_integration_dir, "sojahubrelayerdb"))
        extract("ganache.tar.gz", ganache_db_dir)
        extract("sojahubrelayerdb.tar.gz", relayer_db_path)
        ...
        extract("sojanoded.tar.gz", self.cmd.get_user_home(".sojanoded"))  # Needed for "--keyring-backend test"
    ...
    def restart_processes(self):
        ...
        sojanoded_home = os.path.join(chaindir, ".sojanoded")
        sojanoded_proc = self.cmd.sojanoded_start(tcp_url=self.tcp_url, minimum_gas_prices=(0.5, FURY),
            sojanoded_home=sojanoded_home, trace=True)
...
# Peggy2 environment
# Originally derived from devenv which ran hardhat, sojanoded, ebrelayer like this: cd smart-contracts; GOBIN=~/go/bin npx hardhat run scripts/devenv.ts
class Peggy2Environment(IntegrationTestsEnvironment):
    def __init__(self, cmd: Command, witness_count: int = 1, witness_power: int = 100, consensus_threshold: int = 49,
        ethereum_ws_port: int = 8545, ethereum_chain_id: int = 9999, soja_chain_id = "localnet", use_geth: bool = False
    ):
        super().__init__(cmd)
        ...
        self.chain_id = soja_chain_id
        self.ceth_symbol = sojahub_denom_hash(self.ethereum_chain_id, eth.NULL_ADDRESS)
        assert self.ceth_symbol == "sojaBridge99990x0000000000000000000000000000000000000000"

        # This goes to validator0, i.e. sojanode_validators[0]["address"]
        self.validator_mint_amounts: cosmos.Balance = {
            FURY: 999999 * 10**27,
            self.ceth_symbol: 999999 * 10**21,
            "sojaBridge00030x1111111111111111111111111111111111111111": 137 * 10**16,
            "ibc/FEEDFACEFEEDFACEFEEDFACEFEEDFACEFEEDFACEFEEDFACEFEEDFACEFEEDFACE": 137 * 10**16,
        }
        ...
        self.admin_account_mint_amounts: cosmos.Balance = {
            ...
            "sojaBridge00030x1111111111111111111111111111111111111111": 10 ** 16,
    ...
    def run(self) -> Tuple[subprocess.Popen, subprocess.Popen, subprocess.Popen, List[subprocess.Popen]]:
        ...
        # - build_golang_binaries before start_sojahub
        # - start_hardhat before deploy_smart_contracts
        # - start_sojahub before start_witnesses_and_relayers
        ...
        self.cmd.rmdir(self.cmd.get_user_home(".sojanoded"))  # Purge test keyring backend
        ...
        admin_account_name = "sojanodeadmin"
        ...
        sojanoded_network_dir = self.cmd.tmpdir("sojanodedNetwork")  # Gets written to .vscode/launch.json
        self.cmd.rmdir(sojanoded_network_dir)
        self.cmd.mkdir(sojanoded_network_dir)
        network_config_file, sojanoded_exec_args, sojanoded_proc, tcp_url, admin_account_address, sojanode_validators, \
            sojanode_relayers, sojanode_witnesses, sojanode_validator0_home, chain_dir = \
                self.init_sojahub(sojanoded_network_dir, sojanoded_log_file, self.validator_mint_amounts,
        ...
        log.debug("Admin account address (fury source): {}".format(admin_account_address))  # tokens
        log.debug("Witness count: {}".format(self.witness_count))
        log.debug("Consensus thresholds {}".format(self.consensus_threshold))
        log.debug("Validator 0 address: {}".format(sojanode_validators[0]["address"]))  # mint
        ...
        [relayer0_exec_args], [witness_exec_args] = self.start_witnesses_and_relayers(
            ...
            sojanode_validators, sojanode_relayers, sojanode_witnesses, symbol_translator_file, self.log_level_relayer,
            self.log_level_witness, relayer_extra_args)

        hardhat_scripts.update_validator_power(peggy_sc_addrs["CosmosBridge"], evm_validator_addresses, sojanode_witnesses)
        ...
        _unused_peggy2_environment = {
            "admin": {
                ...
                "home": sojanode_validator0_home,
            },
            "symbol_translator_file": symbol_translator_file,
            "w3_url": w3_url,
            "evm_chain_id": self.ethereum_chain_id,
            "chain_id": self.chain_id,
            "validators": sojanode_validators,  # From yaml file generated by sojagen
            "relayers": sojanode_relayers,
        ...
        self.write_env_files(self.project.project_dir(), self.project.go_bin_dir, peggy_sc_addrs, hardhat_accounts,
            admin_account_name, admin_account_address, sojanode_validator0_home, sojanode_validators, sojanode_relayers,
            sojanode_witnesses, tcp_url, hardhat_bind_hostname, hardhat_port, self.ethereum_chain_id, chain_dir,
            sojanoded_exec_args, relayer0_exec_args, witness_exec_args
        )

        return hardhat_proc, sojanoded_proc, relayer0_proc, witness_procs

    def init_sojahub(self, sojanoded_network_dir: str, sojanoded_log_file: TextIO,
        ...
        try:
            self.cmd.sojagen_create_network(self.chain_id, validator_count, sojanoded_network_dir, network_config_file_path,
        ...
        chain_dir_base = os.path.join(sojanoded_network_dir, "validators", self.chain_id)
        ...
        for validator in validators:
            ...
            sojanoded_home = os.path.join(chain_dir_base, validator_moniker, ".sojanoded")
            sojanode = Sojanoded(self.cmd, home=sojanoded_home)
            self.cmd.sojanoded_peggy2_init_validator(sojanode, validator_moniker, validator_mnemonic, evm_network_descriptor, validator_power)

        # TODO Needs to be fixed when we support more than 1 validator
        validator0 = exactly_one(validators)
        validator0_home = os.path.join(chain_dir_base, validator0["moniker"], ".sojanoded")
        validator0_address = validator0["address"]
        chain_dir = os.path.join(chain_dir_base, validator0["moniker"])

        sojanode = Sojanoded(self.cmd, home=validator0_home, chain_id=self.chain_id)

        # Create an ADMIN account on sojanode with name admin_account_name (e.g. "sojanodeadmin")
        admin_account_address = sojanode.peggy2_add_account(admin_account_name, admin_account_mint_amounts, is_admin=True)

        if self.extra_balances_for_admin_account:
            sojanode.add_genesis_account_directly_to_existing_genesis_json({admin_account_address: self.extra_balances_for_admin_account})

        # TODO Check if sojanoded_peggy2_add_relayer_witness_account can be executed offline (without sojanoded running)
        # TODO Check if sojanoded_peggy2_set_cross_chain_fee can be executed offline (without sojanoded running)

        # Create an account for each relayer
        # Note: "--home" is shared with sojanoded's "--home"
        relayers = [{
            "name": name,
            "address": sojanode.peggy2_add_relayer_witness_account(name, admin_account_mint_amounts,
        ...
        gas_prices = (0.5, FURY)
        # @TODO Detect if sojanoded is already running, for now it fails silently and we wait forever in wait_for_soja_account_up
        sojanoded_exec_args = sojanode.build_start_cmd(tcp_url=tcp_url, minimum_gas_prices=gas_prices,
            log_format_json=True, log_level="debug")
        sojanoded_proc = self.cmd.spawn_asynchronous_process(sojanoded_exec_args, log_file=sojanoded_log_file)

        time_before = time.time()
        self.cmd.wait_for_soja_account_up(validator0_address, tcp_url)
        log.debug("Time for sojanoded to come up: {:.2f}s".format(time.time() - time_before))
        ...
        res = sojanode.peggy2_token_registry_set_registry(registry_json, gas_prices, 1.5, admin_account_address,
            self.chain_id)
        ...
        sojanode.wait_for_last_transaction_to_be_mined()
        ...
        gas_prices = [0.5, FURY]
        gas_adjustment = 1.5
        sojanode.peggy2_set_cross_chain_fee(admin_account_address, self.ethereum_chain_id,
        ...
        # signatures. This delay waits for block production. (See commit 5854d8b6f3970c1254cac0eca0e3817354151853)
        sojanode.wait_for_last_transaction_to_be_mined()
        sojanode.peggy2_update_consensus_needed(admin_account_address, self.ethereum_chain_id, self.consensus_threshold)

        return network_config_file, sojanoded_exec_args, sojanoded_proc, tcp_url, admin_account_address, validators, \
    ...
    def start_witnesses_and_relayers(self, web3_websocket_address: str, hardhat_chain_id: int, tcp_url: str,
        ...
        sojanode_validators: Sequence[Mapping[str, Any]], sojanode_relayers: Sequence[Mapping[str, Any]],
        sojanode_witnesses: Sequence[Mapping[str, Any]], symbol_translator_file: str, log_level_relayer: Optional[str],
        ...
        sojanode_validator0 = exactly_one(sojanode_validators)
        sojanode_validator0_address = sojanode_validator0["address"]

        sojanode_relayer0 = exactly_one(sojanode_relayers)
        sojanode_relayer0_mnemonic = sojanode_relayer0["name"]
        sojanode_relayer0_address = sojanode_relayer0["address"]
        sojanode_relayer0_home = sojanode_relayer0["home"]

        bridge_registry_contract_addr = peggy_sc_addrs["BridgeRegistry"]

        self.cmd.wait_for_soja_account_up(sojanode_validator0_address, tcp_url=tcp_url)  # Required for both relayer and witness
        ...
        relayer0_exec_args = ebrelayer.peggy2_build_ebrelayer_cmd(
            ...
            sojanode_relayer0_mnemonic,
            chain_id=chain_id,
            node=tcp_url,
            sign_with=sojanode_relayer0_address,
            symbol_translator_file=symbol_translator_file,
            ethereum_address=evm_validator0_addr,
            ethereum_private_key=evm_validator0_key,
            keyring_backend="test",
            keyring_dir=sojanode_relayer0_home,
            home=sojanode_relayer0_home,
            log_level=log_level_relayer,
            **relayer_extra_args,
        )

        witness_exec_args = []
        for i, w in enumerate(sojanode_witnesses):
    ...
    def write_env_files(self, project_dir, go_bin_dir, evm_smart_contract_addrs, eth_accounts, admin_account_name,
        admin_account_address, sojanode_validator0_home, sojanode_validators, sojanode_relayers, sojanode_witnesses,
        tcp_url, hardhat_bind_hostname, hardhat_port, hardhat_chain_id, chain_dir, sojanoded_exec_args,
        ...
        def format_soja_account(soja_account):
            return {"account": soja_account["address"], "name": soja_account["name"], "homeDir": soja_account["home"]}
        ...
        environment_json = {
            "contractResults": {
                ...
                "contractAddresses": {
                    ...
                    "furyContract": evm_smart_contract_addrs["Fury"],
            ...
            "sojaResults": {
                "validatorValues": sojanode_validators,
                "adminAddress": format_soja_account({"address": admin_account_address, "name": admin_account_name, "home": sojanode_validator0_home}),
                "relayerAddresses": [format_soja_account(a) for a in sojanode_relayers],
                "witnessAddresses": [format_soja_account(a) for a in sojanode_witnesses],
                # "process": { ... },
                "tcpurl": tcp_url,
            }
        }
        ...
        dot_env = dict_merge({
            ...
            "FURY_SOURCE": admin_account_address,
            ...
            "FURYTOKEN_ADDRESS": evm_smart_contract_addrs["Fury"],
            "BRIDGE_TOKEN_ADDRESS": evm_smart_contract_addrs["Fury"],
            "GOBIN": go_bin_dir,
            "TCP_URL": tcp_url,
            "VALIDATOR_ADDRESS": sojanode_validators[0]["address"],
            "VALIDATOR_CONSENSUS_ADDRESS": sojanode_validators[0]["validator_consensus_address"],
            "VALIDATOR_MENOMONIC": sojanode_validators[0]["mnemonic"],
            "VALIDATOR_MONIKER": sojanode_validators[0]["moniker"],
            "VALIDATOR_PASSWORD": sojanode_validators[0]["password"],
            "VALIDATOR_PUB_KEY": sojanode_validators[0]["pub_key"],
        ...
        } for i, validator in enumerate(sojanode_validators)], *[{
            f"RELAYER_ADDRESS_{i}": relayer["address"],
            f"RELAYER_NAME_{i}": relayer["name"],
        } for i, relayer in enumerate(sojanode_relayers)], *[{
            f"WITNESS_ADDRESS_{i}": witness["address"],
            f"WITNESS_NAME_{i}": witness["name"],
        } for i, witness in enumerate(sojanode_witnesses)])
        ...
        launch_json = {
            ...
            "configurations": [
                {
                    "name": "Python: sojatool",
                    "type": "python",
                    "request": "launch",
                    "program": "${workspaceFolder}/test/integration/framework/sojatool",
                    "args": ["run-env"],
                    ...
                    "name": "Debug Sojanoded",
                    "type": "go",
                    "request": "launch",
                    "mode": "debug",
                    "program": "cmd/sojanoded",
                    # Generally we want to use sojanoded_exec_args, except for:
                    # - here we don't have the initial "sojanoded"
        ...
        for config in launch_json["configurations"]:
            if config["name"].startswith("Debug Relayer"):
                intellij_ebrelayer_configs.append(render_intellij_run_xml(
                    "ebrelayer devenv",
                    " ".join(config["args"]),
                    "github.com/Sojahub/sojanode/cmd/ebrelayer",
                ...
                intellij_witness_configs.append(render_intellij_run_xml(
                    ...
                    "$PROJECT_DIR$/cmd/ebrelayer/main.go",
                    {"ETHEREUM_PRIVATE_KEY": dot_env["ETHEREUM_PRIVATE_KEY"]}))
            elif config["name"].startswith("Debug Sojanoded"):
                intellij_sojanoded_configs.append(render_intellij_run_xml(
                    "sojanoded devenv",
                    " ".join(config["args"]),
                    "github.com/Sojahub/sojanode/cmd/sojanoded",
                    "$PROJECT_DIR$/cmd/sojanoded/main.go",
                    {}))

        intellij_ebrelayer_config = exactly_one(intellij_ebrelayer_configs)
        intellij_witness_config = intellij_witness_configs[0]
        intellij_sojanoded_config = exactly_one(intellij_sojanoded_configs)
        ...
        self.cmd.write_text_file(os.path.join(run_dir, "sojanoded.run.xml"), joinlines(intellij_sojanoded_config))

        return environment_json, dot_env, launch_json, intellij_ebrelayer_config, intellij_witness_config, intellij_sojanoded_config
...
class IBCEnvironment(IntegrationTestsEnvironment):
    ...
    def run(self):
        ...
        # Mnemonics can be generated with "sojagen key generate" or "sojanoded keys mnemonic", but that gives us 24 words
        ...
        sojagen = Sojagen(self.cmd)
        # This does not work - "--keyring-backend" is not supported
        x = sojagen.create_standalone(chainnet0, "chain1", mnemonic, ipaddr0, keyring_backend=None)
