...
from sojatool import eth, truffle, hardhat, run_env, sojahub, cosmos, command
from sojatool.sojahub import FURY, CETH
from sojatool.common import *

# These are utilities to interact with running environment (running agains local ganache-cli/hardhat/sojanoded).
...
sojanode_funds_for_transfer_peggy1 = 10**17  # fury
...
def get_env_ctx(cmd=None, env_file=None, env_vars=None):
    ...
    # Add any Sojahub private keys to test keystore
    soja_user_private_keys = ctx.cmd.project.read_peruser_config_file("soja-keys")
    if soja_user_private_keys:
        available_soja_accounts = ctx.sojanode.keys_list()
        for name, address, mnemonic in [[e["name"], e["address"], e["mnemonic"].split(" ")] for e in soja_user_private_keys]:
            existing_acct = [a for a in available_soja_accounts if a["address"] == address]
            if not existing_acct:
                acct = ctx.sojanode.keys_add(name, mnemonic)
                assert acct["address"] == address, "Invalid address for soja account {}".format(name)
    return ctx

def get_env_ctx_peggy2():
    cmd = run_env.Integrator()

    if "SOJATOOL_ENV_FILE" in os.environ:
        # New-style format (hopefully unified)
        # This is for connecting to peggy2-tempnet etc.
        env_file = os.environ["SOJATOOL_ENV_FILE"]
        env_vars = json.loads(cmd.read_text_file(env_file))
        global_mnemonic = env_vars.get("mnemonic", None)

        sojahub_config = env_vars["sojahub"]
        sojanode_url = sojahub_config["rpc_url"]
        sojanode_chain_id = sojahub_config["chain_id"]
        sojanoded_home = sojahub_config.get("home")
        ...
        fury_source = sojahub_config.get("fury_source")
        if not fury_source:
            if "fury_source_mnemonic" in sojahub_config:
                fury_source_mnemonic = sojahub_config["fury_source_mnemonic"]
            ...
            if fury_source_mnemonic:
                ...
                fury_source = sojahub.mnemonic_to_address(cmd, fury_source_mnemonic)
                sojanoded = sojahub.Sojanoded(cmd, home=sojanoded_home)
                if not [x for x in sojanoded.keys_list() if x["address"] == fury_source]:
                    sojanoded.keys_add(None, fury_source_mnemonic)
        ...
        # For either `sojatool run-env` or `devenv`
        ...
        # furyContract and blocklist). TODO Refactor to unified format (above)
        ...
        fury_source = dot_env_vars["FURY_SOURCE"]

        w3_url = eth.web3_host_port_url(dot_env_vars["ETH_HOST"], int(dot_env_vars["ETH_PORT"]))

        sojanode_url = dot_env_vars["TCP_URL"]
        sojanode_chain_id = "localnet"  # TODO Mandatory, but not present either in environment_vars or dot_env_vars
        assert dot_env_vars["CHAINDIR"] == dot_env_vars["HOME"]
        sojanoded_home = os.path.join(dot_env_vars["CHAINDIR"], ".sojanoded")
    ...
    ceth_symbol = sojahub.sojahub_denom_hash(ethereum_network_descriptor, eth.NULL_ADDRESS)
    ...
    ctx = EnvCtx(cmd, w3_conn, ctx_eth, abi_provider, owner_address, sojanoded_home, sojanode_url, sojanode_chain_id,
    ...
    # TODO These are set in run_env.py:Peggy2Environment.init_sojahub(), specifically "sojanoded tx ethbridge set-cross-chain-fee"
...
def get_env_ctx_peggy1(cmd=None, env_file=None, env_vars=None):
    ...
    if "CHAINNET" in env_vars:
        sojanode_chain_id = env_vars["CHAINNET"]
    elif deployment_name:
        sojanode_chain_id = deployment_name
    else:
        sojanode_chain_id = "localnet"
    ...
    if "FURY_SOURCE" in env_vars:
        fury_source = env_vars["FURY_SOURCE"]
    elif "VALIDATOR1_ADDR" in env_vars:
        fury_source = env_vars["VALIDATOR1_ADDR"]
    else:
        fury_source = None

    ethereum_network_id = int(env_vars.get("ETHEREUM_NETWORK_ID", 5777))

    generic_erc20_contract_name = "SojahubTestToken"
    if "SMART_CONTRACT_ARTIFACT_DIR" in env_vars:
        ...
        if deployment_name == "sojahub-1":
            # Special case for Betanet because SojahubTestToken is not deployed there.
    ...
    sojanode_url = env_vars.get("SOJANODE")  # Defaults to "tcp://localhost:26657"
    sojanoded_home = None  # Implies default ~/.sojanoded
    ...
    ctx = EnvCtx(cmd, w3_conn, ctx_eth, abi_provider, operator_address, sojanoded_home, sojanode_url, sojanode_chain_id,
...
def soja_addr_to_evm_arg(soja_address):
    return soja_address.encode("UTF-8")


class EnvCtx:
    def __init__(self, cmd: command.Command, w3_conn: web3.Web3, ctx_eth: eth.EthereumTxWrapper, abi_provider,
        operator: eth.Address, sojanoded_home: str, sojanode_url: Optional[str], sojanode_chain_id: str,
        ...
        self.sojanode = sojahub.Sojanoded(self.cmd, home=sojanoded_home, node=sojanode_url, chain_id=sojanode_chain_id)
        # Refactoring in progress: moving stuff into separate client that encapsulates things like url, home and chain_id
        self.sojanode_client = sojahub.SojanodeClient(self, self.sojanode, grpc_port=9090)
        ...
        self.sojahub_ethbridge_admin_account = self.fury_source
    ...
    def tx_deploy_new_generic_erc20_token(self, deployer_addr: str, name: str, symbol: str, decimals: int, cosmosDenom: str = None) -> Contract:
        # return self.tx_deploy("SojahubTestToken", self.operator, [name, symbol, decimals])
        if on_peggy2_branch:
            # Use BridgeToken
            assert self.generic_erc20_contract == "BridgeToken"
            if cosmosDenom is None:
                cosmosDenom = "erc20denom"  # TODO Dummy variable since we're using BridgeToken instead of SojahubTestToken

            constructor_args = [name, symbol, decimals, cosmosDenom]
        else:
            # Use SojahubTestToken for TestNet and Devnet, and BridgeToken for Betanet
    ...
    def tx_bridge_bank_lock_eth(self, from_eth_acct, to_soja_acct, amount):
        recipient = soja_addr_to_evm_arg(to_soja_acct)
        bridge_bank = self.get_bridge_bank_sc()
    ...
    def tx_bridge_bank_lock_erc20(self, token_addr, from_eth_acct, to_soja_acct, amount):
        recipient = soja_addr_to_evm_arg(to_soja_acct)
        bridge_bank = self.get_bridge_bank_sc()
    ...
    def tx_bridge_bank_burn_erc20(self, token_addr: str, from_eth_acct: str, to_soja_acct: str, amount: int) -> HexBytes:
        recipient = soja_addr_to_evm_arg(to_soja_acct)
        bridge_bank = self.get_bridge_bank_sc()
        # When transfering ERC20, the amount needs to be passed as argument, and the "message.value" should be 0
        tx_opts = {"value": 0}
    ...
    def tx_approve_and_lock(self, token_sc, from_eth_acct, to_soja_acct, amount):
        bridge_bank_sc = self.get_bridge_bank_sc()
        txhash1 = self.tx_approve(token_sc, self.operator, bridge_bank_sc.address, amount)
        txhash2 = self.tx_bridge_bank_lock_erc20(token_sc.address, from_eth_acct, to_soja_acct, amount)
        log.debug("tx_approve_and_lock: {} '{}' ({}) from {} to {}".format(amount, token_sc.functions.name().call(),
            token_sc.functions.symbol().call(), from_eth_acct, to_soja_acct))
    ...
    def create_new_currency(self, symbol, name, decimals, amount, minted_tokens_recipient):
        ...
        1. Deploys a new instance of SojahubTestToken
        ...
        """
        assert self.generic_erc20_contract == "SojahubTestToken"  # Preserve compatibiliy with integration tests and inflate_tokens.sh
    ...
    def send_eth_from_ethereum_to_sojahub(self, from_eth_addr, to_soja_addr, amount):
        # recipient = to_soja_addr.encode("UTF-8")
    ...
    # TODO Obsolete, use self.bridge_bank_lock_eth()
    def send_erc20_from_ethereum_to_sojahub(self, from_eth_addr, dest_sichain_addr, erc20_token_addr, amount):
    ...
    # TODO Decouple; we want to use this with just "sojanoded" running, move to Sojanoded class?
    def create_sojahub_addr(self, moniker: str = None,
        fund_amounts: Union[cosmos.Balance, cosmos.LegacyBalance, None] = None
    ) -> cosmos.Address:
        """
        Generates a new sojahub address in test keyring. If moniker is given, uses it, otherwise
        generates a random one 'test-xxx'. If fund_amounts is given, the sojahub funds are transferred
        from fury_source to the account before returning.
        """
        moniker = moniker or "test-" + random_string(20)
        acct = self.sojanode.keys_add(moniker)
        soja_address = acct["address"]
        if fund_amounts:
            fund_amounts = cosmos.balance_normalize(fund_amounts)  # Convert from old format if neccessary
            fury_source_balances = self.get_sojahub_balance(self.fury_source)
            for denom, required_amount in fund_amounts.items():
                ...
                assert available_amount >= required_amount, "Fury source {} would need {}, but only has {}".format(
                    self.fury_source, soja_format_amount(required_amount, denom), soja_format_amount(available_amount, denom))
            old_balances = self.get_sojahub_balance(soja_address)
            self.send_from_sojahub_to_sojahub(self.fury_source, soja_address, fund_amounts)
            self.sojanode.wait_for_balance_change(soja_address, old_balances, min_changes=fund_amounts)
            new_balances = self.get_sojahub_balance(soja_address)
            assert cosmos.balance_zero(cosmos.balance_sub(new_balances, fund_amounts))
        return soja_address

    # TODO Clean up
    def send_from_sojahub_to_sojahub(self, from_soja_addr: cosmos.Address, to_soja_addr: cosmos.Address,
        amounts: cosmos.Balance
    ) -> Mapping:
        return self.sojanode.send(from_soja_addr, to_soja_addr, amounts)

    # TODO Clean up
    def get_sojahub_balance(self, soja_addr: cosmos.Address, height: Optional[int] = None,
        disable_log: bool = False, retries_on_error: Optional[int] = None, delay_on_error: int = 3
    ) -> cosmos.Balance:
        return self.sojanode.get_balance(soja_addr, height=height, disable_log=disable_log,
            retries_on_error=retries_on_error, delay_on_error=delay_on_error)

    # TODO Clean up
    def get_current_block(self):
        return self.sojanode.get_current_block()

    # TODO Clean up
    def status(self):
        return self.sojanode.status()

    def eth_symbol_to_soja_symbol(self, eth_token_symbol):
        assert not on_peggy2_branch
        # TODO sojahub.use sojahub_denom_hash() if on_peggy2_branch
        # E.g. "usdt" -> "cusdt"
    ...
    def bridge_bank_lock_eth(self, from_eth_acct, to_soja_acct, amount):
        """ Sends ETH from Ethereum to Sojahub (lock) """
        txhash = self.tx_bridge_bank_lock_eth(from_eth_acct, to_soja_acct, amount)
        return self.eth.wait_for_transaction_receipt(txhash)

    def bridge_bank_lock_erc20(self, token_sc, from_eth_acct, to_soja_acct, amount):
        txhash = self.tx_bridge_bank_lock_erc20(token_sc.address, from_eth_acct, to_soja_acct, amount)
        return self.eth.wait_for_transaction_receipt(txhash)

    def bridge_bank_burn_erc20(self, token_sc: Contract, from_eth_acct: str, to_soja_acct: str, amount: int) -> TxReceipt:
        txhash = self.tx_bridge_bank_burn_erc20(token_sc.address, from_eth_acct, to_soja_acct, amount)
        return self.eth.wait_for_transaction_receipt(txhash)
    ...
    # For Sojahub-native assets (fury) we need to use "burn".
    # Compare: smart-contracts/scripts/test/{sendLockTx.js OR sendBurnTx.js}
    # sendBurnTx is called when sojahub_symbol == "fury", sendLockTx otherwise
    def send_from_ethereum_to_sojahub(self, from_eth_acct: str, to_soja_acct: str, amount: int, token_sc: Contract = None, isLock: bool = True) -> TxReceipt:
        if token_sc is None:
            # ETH transfer
            self.bridge_bank_lock_eth(from_eth_acct, to_soja_acct, amount)
        else:
            # ERC20 token transfer
            self.approve_erc20_token(token_sc, from_eth_acct, amount)
            if isLock:
                self.bridge_bank_lock_erc20(token_sc, from_eth_acct, to_soja_acct, amount)
            else:
                self.bridge_bank_burn_erc20(token_sc, from_eth_acct, to_soja_acct, amount)
    ...
    def sanity_check(self):
        ...
        if on_peggy2_branch:
            ...
            assert (self.sojanode.chain_id != "sojahub-testnet-1") or (bridge_bank_sc.address == "0x6CfD69783E3fFb44CBaaFF7F509a4fcF0d8e2835")
            assert (self.sojanode.chain_id != "sojahub-devnet-1") or (bridge_bank_sc.address == "0x96DC6f02C66Bbf2dfbA934b8DafE7B2c08715A73")
            assert (self.sojanode.chain_id != "localnet") or (bridge_bank_sc.address == "0x30753E4A8aad7F8597332E813735Def5dD395028")
        ...
        available_accounts = self.sojanode.keys_list()
        fury_source_account = [x for x in available_accounts if x["address"] == self.fury_source]
        assert len(fury_source_account) == 1, "There should be exactly one key in test keystore corresponding to " \
            "FURY_SOURCE {}".format(self.fury_source)
        if len(fury_source_account) != 1:
            raise Exception
        fury_source_balance = self.get_sojahub_balance(self.fury_source).get(FURY, 0)
