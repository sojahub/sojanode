...
import string
import random
from dispensation_envutils import create_new_sojaaddr_and_key, send_sample_fury, balance_check, \
...
def test_create_new_claim(claimType):
    sojahub_address, sojahub_name = create_new_sojaaddr_and_key()
    keyring_backend = 'test'
    chain_id = 'localnet'
    from_address = 'sojanodeadmin'
    amount = '100000000010000000fury'
    send_sample_fury(from_address, sojahub_address, amount, keyring_backend, chain_id, "")
    time.sleep(5)
    txnhash = (create_claim(sojahub_address, claimType, keyring_backend, chain_id))
    ...
    try:
        ...
        assert str(expectedOutputTagsList[1]) == sojahub_address
