...
from test_utilities import get_shell_output, get_sojahub_addr_balance, \
    advance_n_ethereum_blocks, n_wait_blocks, \
    send_from_ethereum_to_sojahub
from test_utilities import wait_for_sojahub_addr_balance, \
    get_required_env_var, \
    EthereumToSojahubTransferRequest
...
def test_rollback_chain(source_ethereum_address, solidity_json_path):
    ...
    new_account = burn_lock_functions.create_new_sojaaddr(credentials=credentials, keyname=new_account_key)
    credentials.from_key = new_account["name"]

    # Any amount will work
    amount = 11000

    request = EthereumToSojahubTransferRequest(
        sojahub_address=new_account["address"],
    ...
    burn_lock_functions.transfer_ethereum_to_sojahub(request, 50)
    ...
    initial_user_balance = get_sojahub_addr_balance(new_addr, "", request.sojahub_symbol)
    logging.info(f"initial_user_balance {initial_user_balance}")

    transfer_1 = send_from_ethereum_to_sojahub(transfer_request=request)
    ...
    # the transaction should not have happened on the sojahub side yet
    ...
    second_user_balance = get_sojahub_addr_balance(new_addr, "", request.sojahub_symbol)
    ...
    balance_after_sleep = get_sojahub_addr_balance(new_addr, "", request.sojahub_symbol)
    logging.info(f"get_sojahub_addr_balance after sleep is {balance_after_sleep} for {new_addr}")

    expected_balance = initial_user_balance + request.amount
    logging.info(f"look for a balance of {expected_balance}")
    wait_for_sojahub_addr_balance(new_addr, request.sojahub_symbol, expected_balance, "")
