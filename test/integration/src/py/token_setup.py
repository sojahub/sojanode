...
from integration_env_credentials import create_new_sojaaddr_and_credentials
from test_utilities import create_new_currency


# This file is for setting up an installation with a set of currencies given
# in a json file like ui/core/src/assets.sojahub.mainnet.json.
#
# Run example:
#
#  TOKENS_FILE=$BASEDIR/ui/core/src/assets.sojahub.mainnet.json python3 -m pytest src/py/token_setup.py

def build_request_for_new_sojahub_address(basic_transfer_request, source_ethereum_address, new_currency, amount):
    sojaaddress, credentials = create_new_sojaaddr_and_credentials()
    ...
    request.sojahub_symbol = "c" + new_currency["newtoken_symbol"]
    request.sojahub_address = sojaaddress
...
def test_can_create_a_new_token_and_peg_it(
    ...
    for t in tokens["assets"]:
        if t["symbol"] in existing_tokens or t["symbol"] == "fury":
