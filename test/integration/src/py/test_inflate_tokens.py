...
from sojatool import eth, sojahub
from sojatool.inflate_tokens import InflateTokens
from sojatool.common import *


# Sojahub wallets to which we want to distribute
test_wallets = [
    "soja1fpq67nw66thzmf2a5ng64cd8p8nxa5vl9d3cm4",
    "soja1syavy2npfyt9tcncdtsdzf7kny9lh777yqc2nd",
    "soja1hjkgsq0wcmwdh8pr3snhswx5xyy4zpgs833akh",
    "soja1ypc5qcq5ha562xlak4xw3g6v352k39t6868jhx",
    "soja1u7cp5e5kty8xwuu7k234ah4jsknvkzazqagvl6",
    "soja1lj3rsayj4xtrhp2e3elv4nf7lazxty272zqegr",
    "soja1cffgyxgvw80rr6n9pcwpzrm6v8cd6dax8x32f5",
    "soja1dlse3w2pxlmuvsj5eda344zp99fegual958qyr",
    "soja1m7257566ehx7ya4ypeq7lj4y2h075z6u2xu79v",
    "soja1qrxylp97p25wcqn4cs9nd02v672073ynpkt4yr",
    "soja13rysrrdlhtmuc2pzve7jk0t4pwytwyxhaqcqcn",
    "soja1shywxv2g8gvjcqknvkxu4p6lkqhfclwwj2qk6h",
    "soja1gqm44p5ax4kgk6hksxgv4vuh2adue2acxvg542",
    "soja1zwgc9frcfpt3hhkqfu9u7up94ag5rp30kwrwrj",
]
...
def _test_inflate_tokens_parametrized(ctx, number_of_tokens):
    ...
    balances_before = [ctx.get_sojahub_balance(w) for w in wallets]
    script.transfer(requested_tokens, amount_in_tokens, wallets, amount_gwei)
    balances_delta = [sojahub.balance_delta(balances_before[i], ctx.get_sojahub_balance(w)) for i, w in enumerate(wallets)]

    for balances_delta in balances_delta:
        for t in requested_tokens:
            assert balances_delta[ctx.eth_symbol_to_soja_symbol(t["symbol"])] == amount_in_tokens * 10**t["decimals"]
