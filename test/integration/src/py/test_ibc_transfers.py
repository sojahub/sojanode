...
def test_burn_ibc_coins(
    ...
    request, credentials = generate_test_account(
        ...
        target_fury_balance=sojahub_fees_int * 2 + small_amount
    )
    ...
    request.sojahub_symbol = feedface_token
...
def test_lock_ibc_coins(
    ...
    feedface_transfer_request.sojahub_address = fury_source
    feedface_transfer_request.sojahub_destination_address = request.sojahub_address
    feedface_transfer_request.sojahub_symbol = feedface_token
    feedface_transfer_request.amount = 100

    burn_lock_functions.send_from_sojahub_to_sojahub(feedface_transfer_request, fury_source_integrationtest_env_credentials)
    ...
    request.sojahub_symbol = feedface_token
    request.amount = small_amount
    burn_lock_functions.send_from_sojahub_to_ethereum(request, credentials)
    ...
    logging.info("send FEEDFACE back to sojahub")
    burn_lock_functions.transfer_ethereum_to_sojahub(request, 10)
