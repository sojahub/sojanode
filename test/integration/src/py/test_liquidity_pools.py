...
from test_utilities import get_required_env_var, SojahubcliCredentials, get_optional_env_var, ganache_owner_account, \
    get_shell_output_json, get_shell_output, detect_errors_in_sojanoded_output, get_transaction_result, amount_in_wei, sojanoded_binary
...
def get_pools(sojanoded_node):
    node = f"--node {sojanoded_node}" if sojanoded_node else ""
    command_line = f"{sojanoded_binary} q clp pools {node} --output json"
...
def create_pool(
    ...
    sojahub_fees_entry = f"--fees {transfer_request.sojahub_fees}" if transfer_request.sojahub_fees else ""
    cmd = " ".join([
        yes_entry,
        f"{sojanoded_binary} tx clp create-pool",
        f"--from {transfer_request.sojahub_address}",
        f"--symbol {transfer_request.sojahub_symbol}",
        ...
        node,
        sojahub_fees_entry,
        f"--home {credentials.sojanoded_homedir} ",
        "-y --output json"
    ])
...
def swap_pool(
    ...
    cmd = " ".join([
        yes_entry,
        f"{sojanoded_binary} tx clp swap",
        f"--from {transfer_request.sojahub_address}",
    ...
    txn = get_transaction_result(json_str["txhash"], transfer_request.sojanoded_node, transfer_request.chain_id)
    tx = txn["tx"]
...
def remove_pool_liquidity(
    ...
    cmd = " ".join([
        yes_entry,
        f"{sojanoded_binary} tx clp remove-liquidity",
        f"--from {transfer_request.sojahub_address}",
        f"--symbol {transfer_request.sojahub_symbol}",
        f"--wBasis {wBasis}",
        f"--asymmetry 0",
...
def add_pool_liquidity(
    ...
    cmd = " ".join([
        yes_entry,
        f"{sojanoded_binary} tx clp add-liquidity",
        f"--from {transfer_request.sojahub_address}",
        f"--symbol {transfer_request.sojahub_symbol}",
        f"--nativeAmount {transfer_request.amount}",
        f"--externalAmount {transfer_request.amount}",
...
def test_create_pools(
    ...
    sojaaddress = request.sojahub_address
    # wait for balance
    test_utilities.wait_for_sojahub_addr_balance(sojaaddress, "fury", target_fury_balance, basic_transfer_request.sojanoded_node)
    test_utilities.wait_for_sojahub_addr_balance(sojaaddress, "ceth", target_ceth_balance, basic_transfer_request.sojanoded_node)

    pools = get_pools(basic_transfer_request.sojanoded_node)
    change_amount = 10 ** 18
    basic_transfer_request.amount = change_amount
    basic_transfer_request.sojahub_symbol = "ceth"
    basic_transfer_request.sojahub_address = sojaaddress
    ...
    if pools is None:
        ...
        get_pools(basic_transfer_request.sojanoded_node)
        current_ceth_balance = current_ceth_balance - change_amount
        current_fury_balance = current_fury_balance - change_amount - sojahub_fees_int
        assert(test_utilities.get_sojahub_addr_balance(sojaaddress, basic_transfer_request.sojanoded_node, "fury") == current_fury_balance)
        assert(test_utilities.get_sojahub_addr_balance(sojaaddress, basic_transfer_request.sojanoded_node, "ceth") == current_ceth_balance)

    # check for failure if we try to create a pool twice
    txn = create_pool(basic_transfer_request, credentials)
    assert(txn["code"] == 14)
    get_pools(basic_transfer_request.sojanoded_node)
    current_fury_balance = current_fury_balance - sojahub_fees_int
    assert(test_utilities.get_sojahub_addr_balance(sojaaddress, basic_transfer_request.sojanoded_node, "fury") == current_fury_balance)
    assert(test_utilities.get_sojahub_addr_balance(sojaaddress, basic_transfer_request.sojanoded_node, "ceth") == current_ceth_balance)
...
def test_pools(
    ...
    sojahub_symbol = ("c" + new_currency["newtoken_symbol"]).lower()
    ...
    logging.info(f"transfer some of the new currency {new_currency_symbol} to the test sojahub address")
    request.ethereum_symbol = new_currency["newtoken_address"]
    request.sojahub_symbol = sojahub_symbol
    ...
    # wait for balance
    test_utilities.wait_for_sojahub_addr_balance(sojaaddress, "fury", target_fury_balance, basic_transfer_request.sojanoded_node)
    test_utilities.wait_for_sojahub_addr_balance(sojaaddress, sojahub_symbol, target_new_currency_balance, basic_transfer_request.sojanoded_node)
    ...
    request2.sojahub_symbol = sojahub_symbol
    request2.amount = target_new_currency_balance
    burn_lock_functions.transfer_ethereum_to_sojahub(request2)

    sojaaddress2 = request2.sojahub_address
    # wait for balance
    test_utilities.wait_for_sojahub_addr_balance(sojaaddress2, "fury", target_fury_balance, basic_transfer_request.sojanoded_node)
    test_utilities.wait_for_sojahub_addr_balance(sojaaddress2, sojahub_symbol, target_new_currency_balance, basic_transfer_request.sojanoded_node)

    pools = get_pools(basic_transfer_request.sojanoded_node)
    basic_transfer_request.sojahub_symbol = sojahub_symbol
    basic_transfer_request.sojahub_address = sojaaddress
    ...
    current_fury_balance = current_fury_balance - change_amount - sojahub_fees_int
    assert(test_utilities.get_sojahub_addr_balance(sojaaddress, basic_transfer_request.sojanoded_node, "fury") == current_fury_balance)
    assert(test_utilities.get_sojahub_addr_balance(sojaaddress, basic_transfer_request.sojanoded_node, sojahub_symbol) == current_coin_balance)
    ...
    #assert(test_utilities.get_sojahub_addr_balance(sojaaddress, basic_transfer_request.sojanoded_node, "fury") == current_fury_balance)
    #assert(test_utilities.get_sojahub_addr_balance(sojaaddress, basic_transfer_request.sojanoded_node, sojahub_symbol) == current_ceth_balance)
    # no slippage if pool is perfectly balanced.

    # TODO: compute this precisely?
    slip_pct = 0.01
    balance = test_utilities.get_sojahub_addr_balance(sojaaddress, basic_transfer_request.sojanoded_node, "fury")
    ...
    balance = test_utilities.get_sojahub_addr_balance(sojaaddress, basic_transfer_request.sojanoded_node, sojahub_symbol)
    ...
    basic_transfer_request.amount = 10 ** 19
    txn = swap_pool(basic_transfer_request, "fury", sojahub_symbol, credentials)
    assert(txn["code"] == 27)
    get_pools(basic_transfer_request.sojanoded_node)
    current_fury_balance = current_fury_balance - sojahub_fees_int
    assert(test_utilities.get_sojahub_addr_balance(sojaaddress, basic_transfer_request.sojanoded_node, "fury") == current_fury_balance)
