...
import string
import random
from dispensation_envutils import create_offline_singlekey_txn, create_new_sojaaddr_and_key, send_sample_fury, balance_check, \
...
def test_create_offline_singlekey_txn(claimType):
    ...
    chain_id = 'localnet'
    amount = '100000000000000000fury'
    sampleamount = '1000fury'

    #THESE 4 TXNS ARE TO REGISTER NEW ACCOUNTS ON CHAIN
    send_sample_fury(from_address, runner_address, amount, keyring_backend, chain_id, "--offline")
    time.sleep(5)
    send_sample_fury(from_address, distributor_address, amount, keyring_backend, chain_id, "--offline")
    time.sleep(5)
    send_sample_fury(from_address, destaddress1, sampleamount, keyring_backend, chain_id, "--offline")
    time.sleep(5)
    send_sample_fury(from_address, destaddress2, sampleamount, keyring_backend, chain_id, "--offline")
    time.sleep(5)
    ...
    assert str(msg_type) == '/sojanode.dispensation.v1.MsgCreateDistribution'
...
def test_run_offline_singlekey_txn(claimType):
    ...
    assert str(run_msg_type) == '/sojanode.dispensation.v1.MsgRunDistribution'
