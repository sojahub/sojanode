...
from pytest_utilities import create_new_sojaaddr
from pytest_utilities import generate_test_account, generate_minimal_test_account
from test_utilities import EthereumToSojahubTransferRequest
...
def test_ebrelayer_restart(
    ...
    balance = test_utilities.get_sojahub_addr_balance(request.sojahub_address, request.sojanoded_node, "ceth")
    ...
    time.sleep(5)
    assert balance == test_utilities.get_sojahub_addr_balance(request.sojahub_address, request.sojanoded_node,
                                                               "ceth")
...
def test_ethereum_transactions_with_offline_relayer(
    ...
    amount = 9000
    new_addresses = list(map(lambda x: create_new_sojaaddr(), range(3)))
    ...
    yarn_result = test_utilities.run_yarn_command(
        " ".join([
            ...
            f"--sojahub_address {new_addresses[0]}",
    ...
    for a in new_addresses:
        test_utilities.wait_for_soja_account(a, basic_transfer_request.sojanoded_node, 90)
        test_utilities.wait_for_sojahub_addr_balance(a, "ceth", amount, basic_transfer_request.sojanoded_node, 90)


@pytest.mark.usefixtures("ensure_relayer_restart")
def test_sojahub_transactions_with_offline_relayer(
        basic_transfer_request: EthereumToSojahubTransferRequest,
        fury_source_integrationtest_env_credentials: test_utilities.SojahubcliCredentials,
    ...
    for a in new_eth_addrs:
        ...
        sojahub_balance = test_utilities.get_sojahub_addr_balance(request.sojahub_address, request.sojanoded_node,
                                                                    "ceth")
        logging.info(f"sojahub balance is {sojahub_balance}, request is {request}")
        test_utilities.send_from_sojahub_to_ethereum(
