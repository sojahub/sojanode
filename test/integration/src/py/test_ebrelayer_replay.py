...
from test_utilities import get_required_env_var, get_shell_output, SojahubcliCredentials
...
def build_request(
    ...
    request = EthereumToSojahubTransferRequest(
        sojahub_address=new_addr["address"],
...
def test_transfer_eth_to_ceth_using_replay_blocks(
    ...
    fury_transfer_request.sojahub_symbol = "fury"
    small_amount = 9
    fury_transfer_request.amount = small_amount
    test_utilities.send_from_sojahub_to_ethereum(fury_transfer_request, credentials)
    ...
    logging.info(f"send {small_amount} fury to {request.sojahub_address}")
    test_utilities.send_from_ethereum_to_sojahub(request)
    ...
    time.sleep(5)
    balance_with_no_relayer = test_utilities.get_sojahub_addr_balance(
        request.sojahub_address, request.sojanoded_node,
        request.sojahub_symbol
    )
 ...
 --keyring-backend test --node tcp://0.0.0.0:26657 --from {mon}  --symbol-translator-file {integration_dir}/config/symbol_translator.json"""
    ...
    time.sleep(5)
    logging.info(f"check the ending balance of {request.sojahub_address} after replaying blocks")
    ending_balance = test_utilities.get_sojahub_addr_balance(request.sojahub_address, request.sojanoded_node,
                                                              request.sojahub_symbol)
    ...
    test_utilities.get_shell_output(cmd)
    time.sleep(5)
    ending_balance2 = test_utilities.get_sojahub_addr_balance(request.sojahub_address, request.sojanoded_node,
                                                               request.sojahub_symbol)
