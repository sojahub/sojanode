...
def test_bulk_transfers(
    ...
    new_addresses_and_keys = list(map(lambda x: create_new_sojaaddr_and_key(), range(n_transfers)))
    ...
    for a in new_addresses:
        ...
        test_utilities.wait_for_sojahub_addr_balance(a, "ceth", amount, basic_transfer_request.sojanoded_node, 180)
        test_utilities.wait_for_sojahub_addr_balance(a, "fury", amount, basic_transfer_request.sojanoded_node, 180)
    ...
    for sojaaddr, ethaddr in zip(new_addresses_and_keys, new_eth_addrs):
        r = copy.deepcopy(basic_transfer_request)
        r.sojahub_address = sojaaddr[0]
        r.ethereum_address = ethaddr["address"]
        r.amount = 100
        simple_credentials.from_key = sojaaddr[1]
