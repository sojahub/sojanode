...
from test_utilities import get_required_env_var, SojahubcliCredentials, get_optional_env_var, ganache_owner_account, \
    get_shell_output_json, get_shell_output, detect_errors_in_sojanoded_output, get_transaction_result, amount_in_wei
...
def send_sample_fury(from_address, to_address, amount, keyring_backend, chain_id, offline):
    ...
    sojahub_fees_entry = f"--fees 100000000000000000fury"
...
def create_online_singlekey_txn(
    ...
    sojahub_gas_entry = f"--gas auto"
    sojahub_fees_entry = f"--fees 50000fury"
    keyring_backend_entry = f"--keyring-backend test"
    output = 'output.json'
    cmd = " ".join([
        "sojanoded tx dispensation create",
        f"{claimType}",
        output,
        sojahub_gas_entry,
        sojahub_fees_entry,
...
def create_online_singlekey_async_txn(
    ...
    logging.debug(f"create_online_dispensation")
    sojahub_fees_entry = f"--fees 150000fury"
    sojahub_gas_entry = f"--gas auto --gas-adjustment=1.5"
    keyring_backend_entry = f"--keyring-backend test"
    output = 'output.json'
    cmd = " ".join([
        "sojanoded tx dispensation create",
...
def run_dispensation(
    ...
    sojahub_gas_entry = f"--gas auto --gas-adjustment=1.5"
    sojahub_fees_entry = f"--fees 200000fury"
    keyring_backend_entry = f"--keyring-backend test"
    cmd = " ".join([
        "sojanoded tx dispensation run",
        distribution_name,
        f"{claimType}",
        distribution_count,
...
def query_created_claim(claimType):
    cmd = " ".join([
        "sojanoded q dispensation claims-by-type",
        f"{claimType}",
        "--chain-id localnet",
        f"-o json"
    ])
...
def create_claim(
    ...
    sojahub_fees_entry = f"--fees 100000000000000000fury"
    cmd = " ".join([
        "sojanoded tx dispensation claim",
        f"{claimType}",
        f"--from {sojahub_address}",
        sojahub_fees_entry,
        sojahub_gas_entry,
        f"--chain-id {chain_id}",
        keyring_backend_entry,
