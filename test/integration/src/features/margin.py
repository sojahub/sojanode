...
from sojatool.sojahub import FURY, FURY_DECIMALS
from sojatool import command, environments, project, sojahub, cosmos
...
class TestMargin:
    ...
    def with_test_env(self, pool_setup):
        ...
        margin_params_before = env.sojanoded.query_margin_params()
        env.sojanoded.tx_margin_update_pools(env.clp_admin, mtp_enabled_pools, [], broadcast_mode="block")
        margin_params_after = env.sojanoded.query_margin_params()
        assert len(margin_params_before["params"]["pools"]) == 0
        assert set(margin_params_after["params"]["pools"]) == mtp_enabled_pools

        yield env, env.sojanoded, pool_definitions
        env.close()

    def test_swap_fury_to_external(self):
        with self.with_test_env(self.default_pool_setup) as (env, sojanoded, pools_definitions):
            src_denom = "fury"
            dst_denom = "ceth"
            swap_amount = 10**18
            assert dst_denom in pools_definitions

            account = sojanoded.create_addr()
            env.fund(account, {FURY: 10**25 + swap_amount})
            balance_before = sojanoded.get_balance(account)
            pool_before = sojanoded.query_pools_sorted()[dst_denom]
            sojanoded.tx_clp_swap(account, src_denom, swap_amount, dst_denom, 0, broadcast_mode="block")
            balance_after = sojanoded.get_balance(account)
            pool_after = sojanoded.query_pools_sorted()[dst_denom]
            ...
            assert src_balance_delta == - (sojahub.soja_tx_fee_in_fury + swap_amount)
    ...
    def test_swap_external_to_external(self):
        with self.with_test_env(self.default_pool_setup) as (env, sojanoded, pools_definitions):
            ...
            balances.append(sojanoded.get_balance(account))
            pools.append(sojanoded.query_pools_sorted())
            env.fund(account, {FURY: 10**20, src_denom: swap_amount})

            balance_before = sojanoded.get_balance(account)
            pools_before = sojanoded.query_pools_sorted()
            sojanoded.tx_clp_swap(account, src_denom, swap_amount, dst_denom, 0, broadcast_mode="block")
            balance_after = sojanoded.get_balance(account)
            pools_after = sojanoded.query_pools_sorted()
            ...
            assert fury_delta == -sojahub.soja_tx_fee_in_fury
    ...
    def test_margin(self):
        ...
        with self.with_test_env(self.default_pool_setup) as (env, sojanoded, pools_definitions):
            ...
            margin_params = sojanoded.query_margin_params()

            # sojanoded.tx_margin_whitelist(env.clp_admin, account, broadcast_mode="block")

            pool_before_open = sojanoded.query_pools_sorted()[collateral_asset]
            balance_before_open = sojanoded.get_balance(account)
            mtp_positions_before_open = sojanoded.query_margin_positions_for_address(account)
            res = sojanoded.margin_open_simple(account, borrow_asset, collateral_asset=collateral_asset,
                collateral_amount=collateral_amount, leverage=leverage, position="long")
            mtp_id = int(res["id"])
            pool_after_open = sojanoded.query_pools_sorted()[collateral_asset]
            balance_after_open = sojanoded.get_balance(account)
            mtp_positions_after_open = sojanoded.query_margin_positions_for_address(account)
            ...
            for i in range(10):
                cnt = len(sojanoded.query_margin_positions_for_address(account))
                if cnt == 0:
                    break
            ...
            pool_before_close = sojanoded.query_pools_sorted()[collateral_asset]
            balance_before_close = sojanoded.get_balance(account)
            res2 = sojanoded.tx_margin_close(account, mtp_id, broadcast_mode="block")
            pool_after_close = sojanoded.query_pools_sorted()[collateral_asset]
            balance_after_close = sojanoded.get_balance(account)
