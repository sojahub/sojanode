...
from sojatool import command, cosmos, project, environments, sojahub


# How to use:
# Install Python 3.8-3.10
# Run test/integration/framework/sojatool venv
# Prepare sojanoded binaries
# - Compile sojanoded binary for v0.14.0 and put them into test/integration/framework/build/versions/0.14.0/sojanoded
# - Compile sojanoded binary for v0.15.0-rc.1 and put them into test/integration/framework/build/versions/0.15.0-rc.1/sojanoded
...
# To watch live logs: tail -F /tmp/sojatool.tmp/test_min_commission/sojanoded-0/sojanoded.log
#
# More information about min commission / max voting power:
# Test scenarios (Kevin): https://github.com/Sojahub/sojanode/blob/feature/min-commission/docs/tutorials/commission.md
# Test scenarios (James): https://www.notion.so/sojahub/Minimum-Commissions-Max-Voting-Power-Test-Scenarios-Draft-729620045e2d41f8b18f3a5df28b623b
# How to bypass the 24h limit (Caner): https://www.notion.so/sojahub/v0-15-0-Edit-min-commission-of-existing-validator-edecd16b074a4900974704d223847b48
# Useful info:
# - https://app.zenhub.com/workspaces/current-sprint---engineering-615a2e9fe2abd5001befc7f9/issues/sojahub/sojahub-chainops/200
# Upgrades:
# - https://github.com/Sojahub/sojahub-devops/blob/main/scripts/sojanode/release/testing/upgrade_path.json
# - https://github.com/Sojahub/sojanode/blob/68f69eb7e390363f336ec7a235ab7e564bf5dabb/scripts/upgrade-integration.sh#L39-L39
...
# Kill off any sojanoded processes running from before
def pkill(cmd):
...
def get_binary_for_version(label):
    return project_dir("test", "integration", "framework", "build", "versions", label, "sojanoded")
...
def create_environment(cmd, version, commission_rate=0.06, commission_max_rate=0.10, commission_max_change_rate=0.05,
    ...
    home_root = "/tmp/sojatool.tmp/test_min_commission"
    cmd.rmdir(home_root)
    cmd.mkdir(home_root)

    binary = get_binary_for_version(version)
    assert sojahub.Sojanoded(cmd, binary=binary).version() == version  # Check actual version

    pkill(cmd)

    env = environments.SojanodedEnvironment(cmd, sojanoded_home_root=home_root)
...
def delegate(env, from_index, to_index, amount):
    ...
    sojanoded_to = env._sojanoded_for(to_validator_node_info)
    sojanoded_from_to = env._sojanoded_for(from_validator_node_info, to_node_info=to_validator_node_info)
    validator_addr = sojanoded_to.get_val_address(to_validator_node_info["admin_addr"])
    ...
    res = sojanoded_from_to.staking_delegate(validator_addr, {env.staking_denom: amount}, from_addr, broadcast_mode="block")
...
def test_min_commission_create_new_validator(cmd: command.Command):
    def test_case(commission_rate, commission_max_rate, should_succeed):
        ...
        if should_succeed:
            ...
            assert sojahub.is_min_commission_too_low_exception(exception)
...
def test_min_commission_modify_existing_validator_24h(cmd: command.Command):
    ...
    sojanoded0 = env._sojanoded_for(env.node_info[0])
    admin0_addr = env.node_info[0]["admin_addr"]
    sojanoded1 = env._sojanoded_for(env.node_info[1])
    admin1_addr = env.node_info[1]["admin_addr"]
    sojanoded2 = env._sojanoded_for(env.node_info[2])
    admin2_addr = env.node_info[2]["admin_addr"]

    validators = sojanoded0.query_staking_validators()
    ...
    exception = None
    try:
        res = sojanoded0.staking_edit_validator(0.05, from_acct=admin0_addr, broadcast_mode="block")
        sojahub.check_raw_log(res)
    except Exception as e:
        exception = e
    assert_no_exception(exception)
    ...
    exception = None
    try:
        res = sojanoded1.staking_edit_validator(0.03, from_acct=admin1_addr, broadcast_mode="block")
        sojahub.check_raw_log(res)
    except Exception as e:
        exception = e
    assert sojahub.is_min_commission_too_low_exception(exception)
    ...
    exception = None
    try:
        res = sojanoded2.staking_edit_validator(0.07, from_acct=admin2_addr, broadcast_mode="block")
        sojahub.check_raw_log(res)
    except Exception as e:
        exception = e
    assert_no_exception(exception)
...
def test_min_commission_upgrade_handler(cmd: command.Command):
    def test_case(pre_upgrade_commission_rate, pre_upgrade_commission_max_rate, expected_commission_rate,
        ...
        if should_succeed:
            ...
            # TODO In case of invalid validator setup (commission_rate > commission_max_rate) sojanoded does not start
        ...
        upgrade_height = sojanoded.get_current_block() + 15  # 15 * 5 = 75s > 60s

        commission_rates_before = exactly_one(sojanoded.query_staking_validators())["commission"]["commission_rates"]
        ...
        commission_rates_after = exactly_one(sojanoded.query_staking_validators())["commission"]["commission_rates"]
...
def test_max_voting_power(cmd: command.Command):
    def test_case(from_validator_index, to_validator_index, amount, should_succeed):
        ...
        validator_powers_before = [int(x["tokens"]) for x in sojanoded.query_staking_validators()]
        ...
        validator_powers_after = [int(x["tokens"]) for x in sojanoded.query_staking_validators()]

        if should_succeed:
            assert_no_exception(exception)
            # Check actual vs. expected validator powers.
            # Note: this assertion might fail if "sojanoded query staking validators" returns a list in different order.
            ...
            assert sojahub.is_max_voting_power_limit_exceeded_exception(exception)
            assert validator_powers_after == validator_powers_before

    soja = 0
    akasha = 1

    test_case(akasha, soja, 100, False)  # Current (and projected) voting power too big
    test_case(soja, akasha, 100, True)
    test_case(soja, akasha, 10**23, False)  # Projected voting power too big
