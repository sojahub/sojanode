...
#
# Scenario description: https://www.notion.so/sojahub/Rewards-2-0-Load-Testing-972fbe73b04440cd87232aa60a3146c5
# Ticket: https://app.zenhub.com/workspaces/current-sprint---engineering-615a2e9fe2abd5001befc7f9/issues/sojahub/sojanode/3020
# How to run a validator in multi-node setup:
# - https://docs.sojahub.finance/network-security/validators/running-sojanode-and-becoming-a-validator
# - https://docs.sojahub.finance/developers/tutorials/setup-standalone-validator-node-manually
#
# Requirements: Python (3.9 is best, 3.8 and 3.10 should also work, support for other versions is currently unknownn)
#
# Example usage:
# cd test/load
# ../integration/framework/sojatool venv
...
# $ curl -i "http://rpc-archive.sojahub.finance/block_results?height=9000000"
...
# }
#
# (2) use rocksb, see https://raw.githubusercontent.com/Sojahub/sojahub-devops/1218ff79b22ab2a6bd22b81d6aa4385a247cafc9/scripts/sojanode/testing/sojanode_n_node_network_simulator.py?token=GHSAT0AAAAAABLH7LII5AAWD6YDWG7THBHGYW7DSVA
#
# (3) Exceptions / printing of _debug...
#
# (4) Use parameter rpc.laddr for sojanoded start instead of self.node
...
from sojatool import command, sojahub, project, cosmos, environments, test_utils2
from sojatool.sojahub import FURY, STAKE, FURY_DECIMALS
...
class Test:
    def __init__(self, cmd: command.Command, prj: project.Project, sojanoded_home_root: str):
        self.cmd = cmd
        self.prj = prj
        ...
        # fury). Number of pools == number of tokens. We create this many tokens, and create a pool for each token.
        ...
        self.sojanoded_home_root = sojanoded_home_root
    ...
    def setup(self):
        ...
        self.amount_of_fury_per_wallet = 10000 * 10**18  # TODO How much?
        ...
        if self.wallets_dir is not None:
            ...
            client_home = os.path.join(self.sojanoded_home_root, "sojanoded-client")
        ...
        # minted furys are transferred there, you can see the minting process here: https://github.com/Sojahub/sojanode/blob/master/x/clp/keeper/rewards.go#L54
        # For LPD, we only transfer the existing funds in CLP to the LP's wallet, you can see here: https://github.com/Sojahub/sojanode/blob/8b2f9c45130c79e07555735185fbe1d00279fab0/x/clp/keeper/pool.go#L128
        sojanoded_client = sojahub.Sojanoded(self.cmd, home=client_home)
        ...
        for i in range(self.number_of_wallets):
            ...
            if predefined_wallets:
                ...
                addr = sojanoded_client.create_addr(mnemonic=mnemonic)
        ...
        faucet_balance = cosmos.balance_add({denom: denom_total_supply for denom in self.tokens}, {FURY: 10**25})
        env.init(faucet_balance=faucet_balance, extra_accounts=extra_accounts)
        env.start()

        self.env = env
        sojanoded = env._sojanoded_for(env.node_info[0])
        sojanoded_client = sojahub.Sojanoded(self.cmd, home=client_home, node=sojahub.format_node_url(
            self.env.node_info[0]["host"], self.env.node_info[0]["ports"]["rpc"]), chain_id=env.chain_id)
        sojanoded_client.get_balance_default_retries = 5
        ...
        for addr, denoms in wallets.items():
            account_number, account_sequence = sojanoded.get_acct_seq(addr)
            for denom in denoms:
                res = sojanoded_client.tx_clp_add_liquidity(addr, denom, self.amount_of_liquidity_added_by_wallet,
                    self.amount_of_liquidity_added_by_wallet, account_seq=(account_number, account_sequence))
                sojahub.check_raw_log(res)
                account_sequence += 1
        sojanoded_client.wait_for_last_transaction_to_be_mined()
        self.check_actual_liquidity_providers(sojanoded_client, env.clp_admin, wallets)

        self.sojanoded = sojanoded
        self.sojanoded_client = sojanoded_client

    def run(self):
        sojanoded = self.sojanoded
        sojanoded_client = self.sojanoded_client
        ...
        current_block = sojanoded_client.get_current_block()
        ...
        if self.rewards_duration_blocks > 0:
            reward_params = sojahub.create_rewards_descriptor("RP_1", rewards_start_block, rewards_end_block,
            ...
            sojanoded.clp_reward_period(admin_addr, reward_params)
        ...
        # TODO sojanoded query reward params --node --chain-id (check if/when implemented)

        # Set up LPD policies
        if self.lpd_duration_blocks > 0:
            lppd_params = sojahub.create_lppd_params(lppd_start_block, lppd_end_block, 0.00045, self.lpd_period_mod)
            sojanoded.clp_set_lppd_params(admin_addr, lppd_params)
            sojanoded.wait_for_last_transaction_to_be_mined()
        ...
        # See https://www.notion.so/sojahub/Rewards-2-0-Load-Testing-972fbe73b04440cd87232aa60a3146c5#7392be2c1a034d2db83b9b38ab89ff9e
    ...
    # TODO Refactor - move to Sojanoded
    def wait_for_block(self, block_number: int) -> float:
        sojanoded = self.sojanoded
        current_block = sojanoded.get_current_block()
        prev_block = None
            ...
            if self.block_results_offset is not None:
                if (prev_block is None) or (current_block != prev_block):
                    ...
                    try:
                        ...
                        blk = sojanoded.get_block_results(height=height)
                        histogram = {}
    ...
    def check_actual_liquidity_providers(self, sojanoded, clp_admin, wallets):
        actual_lp_providers = {}
        for denom in self.tokens:
            for lp in sojanoded.query_clp_liquidity_providers(denom):
...
def run_test_case(args):
    ...
    sojanoded_home_root = cmd.tmpdir("sojatool-test.tmp")
    cmd.rmdir(sojanoded_home_root)
    test = Test(cmd, prj, sojanoded_home_root=sojanoded_home_root)
    ...
    try:
        ...
        try:
            ...
            balance = cosmos.balance_format(test.sojanoded[0].get_balance(addr))
