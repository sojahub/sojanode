...
echo "initializing sojanoded and doing initial IBC transfers"
scripts/init-multichain.sh > /dev/null 2>&1
scripts/do_ibc_transfers.sh > /dev/null 2>&1
echo "killing sojanoded"
ps auxww | grep sojanode-1 | egrep -v grep | awk '{ print $2 }' | xargs kill
sleep 60
echo "killing hermes"
killall hermes
