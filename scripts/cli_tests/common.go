...
func getDefaultArgs() Args {
	...
	toAddr, err := sdk.AccAddressFromBech32("did:fury:s1l7hypmqk2yc334vc6vmdwzp5sdefygj2qt269z")
	if err != nil {
		panic(toAddr)
	}

	kr, err := keyring.New("sojahub", "test", os.TempDir(), nil)
	if err != nil {
		panic(err)
	}
	...
	return Args{
		ChainID:          "sojahub-devnet-1",
		GasPrice:         "",
		GasAdjustment:    0,
		Keybase:          kr,
		ChannelID:        "",
		Sender:           accInfo.GetAddress(),
		SojahubReceiver: toAddr,
		CosmosReceiver:   "",
...
func setNetwork(args *Args, network Network) {
	...
	switch args.Network {
	case Devnet:
		args.ChainID = "sojahub-devnet"
	case TestNet:
		args.ChainID = "sojahub-testnet"
	case MainNet:
		args.ChainID = "sojahub"
	case LocalNet:
