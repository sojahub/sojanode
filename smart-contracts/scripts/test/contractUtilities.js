...
function buildProvider(context, argv, logging) {
    ...
    const {getRequiredEnvironmentVariable} = context.require('./sojahubUtilities');
...
async function setAllowance(context, coinDenom, amount, argv, logging, requestParameters) {
    const sojahubUtilities = context.require('./sojahubUtilities');

    if (coinDenom != sojahubUtilities.NULL_ADDRESS) {
        ...
        if (new BN(currentAllowance).lt(new BN(amount))) {
            const approveResult = await newToken.approve(argv.bridgebank_address, sojahubUtilities.SOLIDITY_MAX_INT, requestParameters);
