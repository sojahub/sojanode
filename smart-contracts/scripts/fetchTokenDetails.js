...
const sojanodeDestinationFile = generateTodayFilename({
  directory: "data",
  prefix: "sojanode_mainnet_update",
  extension: "json",
});
...
async function main() {
  ...
  for (let i = 0; i < addressList.length; i++) {
    try {
      ...
      // Now, the sojanode side:
      const sojanodeObj = { ...SOJANODE_MODEL };
      const v1Denom = generateV1Denom(symbol);
      sojanodeObj.decimals = decimals;
      sojanodeObj.base_denom = v1Denom;
      sojanodeObj.denom = v1Denom;
      sojanodeList.push(sojanodeObj);
  ...
  fs.writeFileSync(
    sojanodeDestinationFile,
    JSON.stringify(sojanodeList, null, 2)
  );
  ...
  print(
    "magenta",
    `Sojanode results have been written to ${sojanodeDestinationFile}.`
  );
  print(
    "yellow",
