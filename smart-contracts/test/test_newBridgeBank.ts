...
import {
    ...
    BridgeTokenSetup,
    FuryContract,
    SojahubContractFactories
} from "../src/tsyringe/contracts";
import {BridgeBank, BridgeBank__factory, BridgeToken} from "../build";
import {SojahubAccounts, SojahubAccountsPromise} from "../src/tsyringe/sojahubAccounts";
import {
    DeploymentChainId,
    DeploymentDirectory,
    DeploymentName,
...
import {impersonateAccount, setupSojahubMainnetDeployment} from "../src/hardhatFunctions";
...
describe("BridgeBank", () => {
    ...
    describe("locking and burning", function () {
        ...
        const invalidRecipient = web3.utils.utf8ToHex("edid:fury:s1nx650s8q9w28f2g3t9ztxyg48ugldptuwzpace")

        before('create test token', async () => {
            accounts = await container.resolve(SojahubAccountsPromise).accounts
            sender = accounts.availableAccounts[0]
            amount = hardhat.ethers.utils.parseEther("100") as BigNumber
            smallAmount = amount.div(100)
            const testTokenFactory = (await container.resolve(SojahubContractFactories).bridgeToken).connect(sender)
        ...
        it("should read the whitelist", async () => {
            const btf = await container.resolve(SojahubContractFactories).bridgeToken
