...
contract("BridgeBank", function (accounts) {
  ...
  describe("Bridge token minting (for burned Cosmos assets)", function () {
    ...
    it("should return true if a sojahub address prefix is correct", async function () {
      (await this.bridgeBank.verifySojaPrefix(this.sender)).should.be.equal(true);
    })

    it("should return false if a sojahub address has an incorrect `soja` prefix", async function () {
      const incorrectSojaAddress = web3.utils.utf8ToHex(
        "eif1nx650s8q9w28f2g3t9ztxyg48ugldptuwzpace"
      );
      (await this.bridgeBank.verifySojaPrefix(incorrectSojaAddress)).should.be.equal(false);
    })
  ...
  describe("Bridge token deposit locking (Ethereum/ERC20 assets)", function () {
    ...
    it("should not allow users to lock ERC20 tokens if the sojaaddress length is incorrect", async function () {
      const invalidSojaAddress = web3.utils.utf8ToHex(
    ...
    it("should not allow users to lock ERC20 tokens if the sojaaddress prefix is incorrect", async function () {
      const invalidSojaAddress = web3.utils.utf8ToHex(
  ...
  // cosmos assets on sojahub, and then we hook into an existing ERC20 contract on mainnet
  ...
  describe("Bridge token creation", function () {
    ...
    it("should create eFury mock and connect it to the cosmos bridge with admin API", async function () {
      const symbol = "eFury"
    ...
    it("should burn eFury to create fury on sojahub", async function () {
    ...
    it("should NOT burn eFury to create fury on sojahub if user is blocklisted", async function () {
      // Add sender to the blocklist
    ...
    it("should mint eFury to transfer Fury from sojahub to ethereum", async function () {
      function convertToHex(str) {
        let hex = '';
        for (let i = 0; i < str.length; i++) {
            hex += '' + str.charCodeAt(i).toString(16);
      ...
      const cosmosSender = "0x" + convertToHex("did:fury:s12qfvgsq76eghlagyfcfyt9md2s9nunsn40zu2h");
      const senderSequence = 1
      const symbol = 'eFury'
      const amount = 100000;
      const nonce = 1;

      // operator should not have any eFury
