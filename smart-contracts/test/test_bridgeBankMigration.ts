...
import {SojahubContractFactories} from "../src/tsyringe/contracts";
...
import {
    getProxyAdmin,
    impersonateAccount,
    setupSojahubMainnetDeployment,
    startImpersonateAccount
} from "../src/hardhatFunctions"
import {SojahubAccountsPromise} from "../src/tsyringe/sojahubAccounts";
...
describe("BridgeBank and CosmosBridge - updating to latest smart contracts", () => {
    ...
    before('use mainnet data', async () => {
        await setupSojahubMainnetDeployment(container, hardhat)
    })
    ...
    describe("upgraded BridgeBank", async () => {
        it("should maintain existing stored values", async () => {
            ...
            const bridgeBankFactory = await container.resolve(SojahubContractFactories).bridgeBank
        ...
        it("should lock and burn via existing validators", async () => {
            ...
            await impersonateAccount(hardhat, upgradeAdmin, hardhat.ethers.utils.parseEther("10"), async fakeDeployer => {
                ...
                const accounts = await container.resolve(SojahubAccountsPromise).accounts
                const bridgeBankFactory = await container.resolve(SojahubContractFactories).bridgeBank
                ...
                const cosmosBridgeFactory = (await container.resolve(SojahubContractFactories).cosmosBridge).connect(fakeDeployer)
                ...
                const testTokenFactory = (await container.resolve(SojahubContractFactories).bridgeToken).connect(operator)
                ...
                }
                {
                    ...
                    const recipient = web3.utils.utf8ToHex("did:fury:s1nx650s8q9w28f2g3t9ztxyg48ugldptuwzpace")
...
async function executeNewProphecyClaimWithTestValues(
    ...
    const cosmosSender = web3.utils.utf8ToHex("did:fury:s1nx650s8q9w28f2g3t9ztxyg48ugldptuwzpace")
