    ...
    Pausable
{
    ...
    /*
     * @dev: Modifier to only allow valid soja addresses
     */
    modifier validSojaAddress(bytes memory _sojaAddress) {
        require(_sojaAddress.length == 42, "Invalid len");
        require(verifySojaPrefix(_sojaAddress) == true, "Invalid soja address");
        _;
    }
    ...
    /*
     * @dev: function to validate if a soja address has a correct prefix
     */
    function verifySojaPrefix(bytes memory _sojaAddress)
        public
        pure
        returns (bool)
    {
        bytes3 sojaInHex = 0x736966;

        for (uint256 i = 0; i < sojaInHex.length; i++) {
            if (sojaInHex[i] != _sojaAddress[i]) {
                return false;
            }
        }
        return true;
