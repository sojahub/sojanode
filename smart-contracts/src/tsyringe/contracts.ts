...
import {SojahubAccounts, SojahubAccountsPromise} from "./sojahubAccounts";
import {
    BridgeBank,
    BridgeBank__factory,
    BridgeRegistry,
...
@singleton()
export class SojahubContractFactories {
...
export class CosmosBridgeProxy {
    ...
    constructor(
        ...
        sojahubContractFactories: SojahubContractFactories,
        cosmosBridgeArgumentsPromise: CosmosBridgeArgumentsPromise,
    ) {
        this.contract = sojahubContractFactories.cosmosBridge.then(async cosmosBridgeFactory => {
...
export function defaultCosmosBridgeArguments(sojahubAccounts: SojahubAccounts, power: number = 100): CosmosBridgeArguments {
    const powers = sojahubAccounts.validatatorAccounts.map(_ => power)
    const threshold = powers.reduce((acc, x) => acc + x)
    return new CosmosBridgeArguments(
        new NotNativeCurrencyAddress(sojahubAccounts.operatorAccount.address),
        threshold,
        sojahubAccounts.validatatorAccounts.map(x => new NotNativeCurrencyAddress(x.address)),
        powers
    )
}
...
@registry([
    {
        ...
        useFactory: instanceCachingFactory<CosmosBridgeArgumentsPromise>(c => {
            const accountsPromise = c.resolve(SojahubAccountsPromise)
...
export class BridgeBankArguments {
    constructor(
        ...
        private readonly sojahubAccountsPromise: SojahubAccountsPromise
    ) {
    }

    async asArray() {
        const cosmosBridge = await this.cosmosBridgeProxy.contract
        const accts = await this.sojahubAccountsPromise.accounts
        const result = [
...
export class BridgeBankProxy {
    ...
    constructor(
        ...
        private sojahubContractFactories: SojahubContractFactories,
        private bridgeBankArguments: BridgeBankArguments,
    ) {
        this.contract = sojahubContractFactories.bridgeBank.then(async bridgeBankFactory => {
...
export class BridgeRegistryProxy {
    ...
    constructor(
        ...
        this.contract = sojahubContractFactories.bridgeRegistry.then(async bridgeRegistryFactory => {
...
export class FuryContract {
    ...
    constructor(
        ...
        this.contract = sojahubContractFactories.bridgeToken.then(async bridgeToken => {
...
export class BridgeTokenSetup {
    ...
    private async build(
        ...
        sojahubAccounts: SojahubAccountsPromise
    ) {
        const efury = await fury.contract
        const owner = (await sojahubAccounts.accounts).ownerAccount
        ...
        const accounts = await sojahubAccounts.accounts
    ...
    constructor(
        ...
        this.complete = this.build(fury, bridgeBankProxy, sojahubAccounts)
    }
}
