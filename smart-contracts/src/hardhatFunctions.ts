...
export const eFuryMainnetAddress = "0x07bac35846e5ed502aa91adf6a9e7aa210f2dcbe"
...
export async function setupDeployment(c: DependencyContainer) {
    ...
    switch(deploymentName) {
        case "sojahub":
        case "sojahub-1":
            setupSojahubMainnetDeployment(c, hre, deploymentName)
            break
        case undefined:
            break
        default:
...
export async function setupSojahubMainnetDeployment(c: DependencyContainer, hre: HardhatRuntimeEnvironment, deploymentName: "sojahub" | "sojahub-1") {
    ...
    // BridgeToken for Fury doesn't have a json file in deployments, so we need to build DeployedBridgeToken by hand
    // instead of using
    const existingFuryToken: BridgeToken = await bridgeTokenFactory.attach(eFuryMainnetAddress)
    const syntheticBridgeToken = {
        contract: Promise.resolve(existingFuryToken),
